...
delivery fleet (10 drivers, 10 bikes, 2 cars, ~300 deliveries/day), and central
...
| --- | --- |
| `stock_inventory_management` | per-shop ingredient/product/supply stock, reorder thresholds, waste and transfers between bakeries |
| `delivery_dispatch_tracking` | orders, driver and vehicle assignment (bike/car), delivery state from pickup to drop-off |
| `document_knowledge_qa` | uploaded unit price lists and operating procedures, and answers over them |
| `fleet_cost_and_pricing` | delivery cost, vehicle operating cost and product pricing from the unit prices on file |
| `management_reporting_dashboard` | chain-wide sales, stock, delivery and fleet KPIs |
| `user_roles_workforce` | users, roles, shop assignment and driver shifts (one user per shop) |
| `operational_procedures_readiness` | procedures turned into daily checklists and tracked acknowledgments |
| `audit_evidence_trail` | auditable trail of stock, delivery, upload and acknowledgment events with integrity hashes |
