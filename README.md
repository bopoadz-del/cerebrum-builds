...
Cerebrum-builds session for **Productivity Platform** — a tiny single-page notes app: save a note with a title and body, list notes, search notes by keyword, delete a note.

Manufactured from vendored Store blocks (session `sess_6d15e13638e24d2d`):

- `productivity_core` — GENERATE notes kernel (no block ids)
- `audit` — REUSE `audit` (action=`log`)
