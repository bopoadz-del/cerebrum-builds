"""Handler for capability analytics_and_reporting.
...
ENTITY = 'analytics_and_reporting'
BLOCK_IDS = ['analytics', 'dashboard', 'portfolio_rollup', 'event_bus']
...
BLOCK_DEFAULT_ACTIONS = {'analytics': 'track_event', 'dashboard': 'render', 'portfolio_rollup': 'aggregate', 'event_bus': 'publish'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'report_name', 'metric_name', 'metric_value', 'period_start', 'period_end', 'appointments_count', 'revenue_total', 'event_type']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record = dict(payload)
        record["report_name"] = str(record.get("report_name") or "").strip()
        record["metric_name"] = str(record.get("metric_name") or "").strip()
        if not record["report_name"] or not record["metric_name"]:
            return {"ok": False, "capability": CAPABILITY_ID,
                    "error": "Missing required fields: report_name, metric_name"}
        results = {}
        errors = {}
        steps = [{
            "block": "event_bus",
            "action": "publish",
            "input": {
                "topic": 'analytics_and_reporting.recorded',
                "payload": {"reference": payload.get("reference") or payload.get("pet_name") or "record"},
                "message": 'analytics_and_reporting recorded',
                "channel": "mcp",
                "tool": "event_bus",
            },
        }]
