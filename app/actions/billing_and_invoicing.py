"""Handler for capability billing_and_invoicing.
...
ENTITY = 'billing_and_invoicing'
BLOCK_IDS = ['portfolio_rollup', 'formula_executor', 'document_engine', 'validation']
...
BLOCK_DEFAULT_ACTIONS = {'portfolio_rollup': 'aggregate', 'formula_executor': 'auto', 'document_engine': 'parse', 'validation': 'validate_pipeline'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'owner_name', 'service_code', 'invoice_total', 'amount_paid', 'currency', 'issued_on', 'due_on', 'payment_method', 'attachment_path']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record["service_code"] = str(record.get("service_code") or "").strip()
        if not record["owner_name"] or not record["service_code"]:
            return {"ok": False, "capability": CAPABILITY_ID,
                    "error": "Missing required field: service_code"}
        total = record.get("invoice_total")
        paid = record.get("amount_paid")
        ...
        record["currency"] = record.get("currency") or "USD"
