"""Clinic performance analytics for VetClinicOS.
...
Domain: dashboards and reports over patient outcomes, revenue and inventory
trends, one metric at a time.

Blocks (Registry-verified, dispatched by keyword action):
  dashboard        render    — the clinic dashboard the metric lands on
  analytics        track_event — the metric sample
  portfolio_rollup aggregate — the rolled-up view across periods

Scope: READS caller input, STORAGE_PATH, vendored blocks. WRITES the
analytics entity through store.save(ENTITY, ...). NEVER network I/O.
"""
...
CAPABILITY_FIELDS = ['reference', 'status', 'metric_name', 'metric_value', 'period', 'period_start', 'period_end', 'dimension']
...
def _block_input(block_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """The object each bound block is handed, built from the metric."""
    metric = _text(payload, "metric_name") or "clinic_metric"
    value = _float(payload, "metric_value", 0.0)
    window = " ".join(
        part for part in (_text(payload, "period_start"), _text(payload, "period_end")) if part
    ) or (_text(payload, "period") or "current")
    if block_id == "dashboard":
        return {"status": _text(payload, "status"), "theme": "grid", "refresh_interval": 30}
    if block_id == "analytics":
        return {"metric": metric, "value": value, "name": metric, "period": window}
...
def _trend(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Metric and its window, as the dashboard reads them."""
    return {
        "metric_name": _text(payload, "metric_name") or "clinic_metric",
        "metric_value": _float(payload, "metric_value", 0.0),
        "period": _text(payload, "period") or "current",
        "dimension": _text(payload, "dimension") or "clinic",
    }
