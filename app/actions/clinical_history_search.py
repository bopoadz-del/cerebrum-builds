"""Handler for capability clinical_history_search.
...
ENTITY = 'clinical_history_search'
BLOCK_IDS = ['database', 'analytics']
...
BLOCK_DEFAULT_ACTIONS = {'database': 'query', 'analytics': 'track_event'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'patient_name', 'tooth', 'procedure', 'provider', 'visit_date', 'date_from', 'date_to', 'search_notes']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record["tooth"] = str(record.get("tooth") or "")
        record["procedure"] = str(record.get("procedure") or "")
        record["provider"] = str(record.get("provider") or "")
        record["search_notes"] = str(record.get("search_notes") or "")
        # Chair-side filter summary over the visit history index.
        window = []
        if record.get("date_from"):
            window.append("from " + str(record["date_from"]))
        if record.get("date_to"):
            window.append("to " + str(record["date_to"]))
        record["history_query"] = " ".join(
            part for part in (
                record["patient_name"], record["tooth"], record["procedure"],
                record["provider"], " ".join(window),
            ) if part
        ).strip()
        results = {}
        errors = {}
