"""Compliance audit for LexManage.
...
Domain: Compliance and audit: immutable audit log entry, validated control rule, readiness score and evidence verification.

Blocks (registry-verified, dispatched by keyword action):
  audit            log              Ensures regulatory compliance and provides audit trails for all actions within the system.
  validation       validate_pipeline Ensures regulatory compliance and provides audit trails for all actions within the system.
  readiness_engine score            Ensures regulatory compliance and provides audit trails for all actions within the system.
  evidence_verifier verify           Ensures regulatory compliance and provides audit trails for all actions within the system.
...
BLOCK_IDS = ['audit', 'validation', 'readiness_engine', 'evidence_verifier']
#: Each block's declared default action (from its block.json).
#: Blocks are action-dispatched; calling one with no action answers
#: 'Unknown action'.
BLOCK_DEFAULT_ACTIONS = {'audit': 'log', 'validation': 'validate_pipeline', 'readiness_engine': 'score', 'evidence_verifier': 'verify'}
#: This capability's own domain columns, declared so the kernel does
#: not strip a domain field that shares a trust-scope name.
CAPABILITY_FIELDS = ['reference', 'status', 'control_id', 'framework', 'regulation', 'event_type', 'actor', 'actor_role', 'action_taken', 'evidence_ref', 'findings', 'occurred_at']
...
def _evidence_digest(payload: Dict[str, Any]) -> str:
    ...
    material = "|".join(
        _text(payload, name)
        for name in ("reference", "control_id", "framework", "action_taken", "evidence_ref")
    )
    return hashlib.sha256(material.encode("utf-8")).hexdigest()


def _readiness_input(payload: Dict[str, Any]) -> Dict[str, Any]:
    """The posture checks the readiness engine scores."""
    framework = _text(payload, "framework") or "internal"
    ...
    return {
        ...
        "control_id": control,
        "framework": framework,
        "evidence_present": bool(_text(payload, "evidence_ref")),
        "findings_recorded": bool(findings),
        "audit_logged": True,
        "checks": [
            "audit trail written",
            "control rule validated",
            "evidence referenced",
            "findings recorded" if findings else "no findings recorded",
        ],
    }
...
def _block_input(block_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    action = _text(payload, "action_taken") or _text(payload, "event_type") or "review"
    framework = _text(payload, "framework") or "internal"
    if block_id == "audit":
        return {
            "event_action": action,
            "resource": control,
            "user_id": actor,
            "category": _text(payload, "actor_role") or "compliance",
            "details": {
                "reference": reference,
                "framework": framework,
    ...
    if block_id == "readiness_engine":
        return {"input": _readiness_input(payload), "reference": reference}
...
def _insight(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Compliance state: the control, its framework and the evidence digest."""
    return {
        "control_id": _text(payload, "control_id") or "unassigned-control",
        "framework": _text(payload, "framework") or "internal",
        "evidence_digest": _evidence_digest(payload),
        "evidence_present": bool(_text(payload, "evidence_ref")),
        "findings": _text(payload, "findings") or "none recorded",
    }

def _normalise_control(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Compliance normalisation: the control identity is canonicalised."""
    data = dict(payload) if isinstance(payload, dict) else {}
    data["control_id"] = (_text(data, "control_id") or "unassigned-control").upper()
    data["framework"] = _text(data, "framework") or "internal"
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload = _normalise_control(payload)
