"""Handler for capability day_before_email_reminders.
...
ENTITY = 'day_before_email_reminders'
BLOCK_IDS = ['notification', 'queue', 'event_bus', 'audit']
...
BLOCK_DEFAULT_ACTIONS = {'notification': 'send', 'queue': 'enqueue', 'event_bus': 'publish', 'audit': 'log'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'patient_name', 'patient_email', 'reminder_date', 'appointment_at', 'reminder_channel', 'message_body', 'delivery_state']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record["reminder_date"] = str(record.get("reminder_date") or "").strip()
        if not record["patient_name"] or not record["patient_email"]:
            return {"ok": False, "capability": CAPABILITY_ID,
                    "error": "Missing required fields: patient_name, patient_email"}
        if record.get("delivery_state") == "cancelled":
            # A cancelled slot is never reminded: no send, no queue entry.
            record["message_body"] = ""
            return {
                "ok": True,
                ...
                "entity": ENTITY,
                "record": record,
                "results": {"skipped": "cancelled appointment"},
            }
        record.setdefault("delivery_state", "queued")
        record["reminder_channel"] = record.get("reminder_channel") or "email"
        # The notification block needs a postal target for the email channel.
        record["to"] = record["patient_email"]
        record["message_body"] = str(record.get("message_body") or (
            "Reminder: you have a dental appointment tomorrow at "
            + str(record.get("appointment_at") or "your scheduled time")
        ))
        results = {}
        errors = {}
        steps = [{
            ...
            "action": "publish",
            "input": {
                "topic": "appointment.reminder.day_before",
                "payload": {
                    "reference": payload.get("reference") or "reminder",
                    "patient_email": record["patient_email"],
                    "reminder_date": record["reminder_date"],
                },
                "message": "day-before reminder queued for delivery",
                "channel": "mcp",
                "tool": "event_bus",
            },
        }]
