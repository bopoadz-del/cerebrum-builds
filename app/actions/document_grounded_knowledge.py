"""Handler for capability document_grounded_knowledge.
...
ENTITY = 'document_grounded_knowledge'
BLOCK_IDS = ['knowledge', 'vector_search', 'document_engine', 'memory', 'spec_analyzer', 'validation']
...
#: "Unknown action: None".
BLOCK_DEFAULT_ACTIONS = {'knowledge': 'search', 'vector_search': 'search', 'document_engine': 'parse', 'memory': 'get', 'spec_analyzer': 'analyze', 'validation': 'check_code_quality'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'branch', 'question', 'document_type', 'answer', 'citations', 'confidence']


_REQUIRED_FIELDS = ['reference', 'status', 'branch', 'question', 'document_type']



def _grounded_answer(record):
    """Answer strictly from the branch's own uploaded documents."""
    from app import llm, retrieval

    question = str(record.get('question') or '')
    tenant_id = str(record.get('branch') or 'local')
    try:
...
def _platform_side(record, unavailable):
    """The platform's own retrieval answers when a block cannot be loaded."""
    answer = _grounded_answer(record) if 'knowledge' in unavailable else None
    ingested = None
    document = record.get('document')
    if isinstance(document, str) and document.strip():
        from app import retrieval

        ingested = retrieval.ingest(tenant_id=str(record.get('branch') or 'local'),
                                    text=document, document=str(record.get('reference') or 'uploaded-document'),
                                    document_type=str(record.get('document_type') or 'procedure'),
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Answers drawn from the bakery's own uploaded documents, tenant by tenant."""
