...
Pipeline:
  ...
  * ``capture``         — extracts the document's fields (title, type, prices)
    so unit prices and delivery procedures are addressable.
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for block_id in BLOCK_IDS:
        ...
        if block_id == "capture":
            prepared["text"] = identity["text"]
            prepared["reference"] = identity["doc_id"]
            prepared["document_type"] = str(data.get("document_type") or "other")
