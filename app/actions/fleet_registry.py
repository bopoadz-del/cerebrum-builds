"""Handler for capability fleet_registry.
...
ENTITY = 'fleet_registry'
BLOCK_IDS = ['database', 'storage', 'audit']
#: Each block's declared default action. Blocks are action-dispatched;
#: calling one with no action is answered with an error.
BLOCK_DEFAULT_ACTIONS = {'database': 'query', 'storage': 'store', 'audit': 'log'}
#: This capability's own domain columns.
CAPABILITY_FIELDS = ['plate_number', 'vin', 'branch', 'category', 'vehicle_state', 'mileage', 'service_history', 'attachment_path', 'reference', 'status']
...
def _domain_record(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Vehicle registry row: identity, branch, state and service posture."""
    record = {k: v for k, v in (payload or {}).items() if v is not None}
    state = str(record.get("vehicle_state") or "available").strip().lower()
    if state not in ("available", "rented", "in_maintenance", "off_road"):
        state = "available"
    record["vehicle_state"] = state
    record["lifecycle_open"] = state in ("available", "rented")
    record["odometer_band"] = (
        "low" if _as_int(record.get("mileage") or 0) < 50000
        else "mid" if _as_int(record.get("mileage") or 0) < 150000
        else "high"
    )
    record["fleet_summary"] = "%s %s at %s (%s, %s km)" % (
        record.get("plate_number") or "unplated",
        record.get("category") or "uncategorised",
        record.get("branch") or "unassigned",
        state,
        _as_int(record.get("mileage") or 0),
    )
    return record
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(result, dict) and result.get('ok') is not False:
        ...
        result = dict(result)
        result['summary'] = _record.get('fleet_summary') or result.get('capability')
