"""Handler for capability invoicing.
...
ENTITY = 'invoicing'
BLOCK_IDS = ['database', 'formula_executor', 'document_engine', 'validation']
...
BLOCK_DEFAULT_ACTIONS = {'database': 'query', 'formula_executor': 'auto', 'document_engine': 'parse', 'validation': 'validate_pipeline'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'patient_name', 'invoice_number', 'treatment_code', 'amount', 'tax_rate', 'total_due', 'payment_status', 'issued_on', 'due_on', 'notes']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record["invoice_number"] = str(
            record.get("invoice_number") or record.get("reference") or ""
        ).strip()
        record["treatment_code"] = str(record.get("treatment_code") or "").strip()
        record["payment_status"] = str(record.get("payment_status") or "unpaid")
        record["issued_on"] = str(record.get("issued_on") or "").strip()
        record["due_on"] = str(record.get("due_on") or "").strip()
        ...
        try:
            ...
            amount = 0.0
        try:
            tax_rate = float(record.get("tax_rate") or 0.0)
        except (TypeError, ValueError):
            tax_rate = 0.0
        record["amount"] = amount
        record["tax_rate"] = tax_rate
        # Fee-schedule arithmetic is declared here; formula_executor is the
        # authority that evaluates it (this handler never invents a total).
        record["formula"] = "amount * (1 + tax_rate / 100)"
        record["variables"] = {"amount": amount, "tax_rate": tax_rate}
        record.setdefault("total_due", round(amount * (1 + tax_rate / 100.0), 2))
        record["filename"] = f"invoice_{record['invoice_number'] or record['reference']}.txt"
        record["content"] = (
            f"Invoice {record['invoice_number']} for {name}: "
            f"{amount:.2f} plus {tax_rate:.2f}% tax"
        )
        results = {}
        errors = {}
