...
#: table written by ``record_guest_check_in`` and filters it to the requested day.
SOURCE_TABLE = "record_guest_check_in"
SOURCE_DATE_FIELD = "checked_in_at"
#: The check-in table's declared columns. The vendored ``database`` block
#: auto-creates ``CREATE TABLE IF NOT EXISTS <table> (id INTEGER PRIMARY KEY)``
#: when a SELECT hits a missing table, which then refuses the record
#: capability's insert (no column named ...). The desk read therefore creates
#: the real schema first, so a read can never poison the write.
SOURCE_COLUMNS = (
    "reference",
    "guest_name",
    "room_number",
...
def _ensure_source_table_inputs() -> Dict[str, Any]:
    """Create the check-in table with its declared columns (idempotent)."""
    return {
        "table": SOURCE_TABLE,
        "schema": {column: "TEXT" for column in SOURCE_COLUMNS},
    }
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Create the check-in schema before reading it: a SELECT over a missing
    # table has the block mint an id-only table that no insert can fill.
    results: Dict[str, Any] = {
        "database_schema": _run_block(
            "database", _ensure_source_table_inputs(), DATABASE_CREATE_ACTION
        )
    }
    errors: Dict[str, str] = {}
