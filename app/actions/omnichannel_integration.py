"""Omnichannel integration for RetailOS.
...
Domain: Connect physical stores, e-commerce platforms, and marketplaces for unified inventory and order management.
...
BLOCK_IDS = ['event_bus', 'workflow', 'queue', 'knowledge']
...
#: 'Unknown action'.
BLOCK_DEFAULT_ACTIONS = {'event_bus': 'publish', 'workflow': 'run', 'queue': 'enqueue', 'knowledge': 'ask'}
#: This capability's own domain columns, declared so the kernel does
#: not strip a domain field that shares a trust-scope name.
CAPABILITY_FIELDS = ['reference', 'status', 'integration_name', 'channel', 'external_order_id', 'sync_direction', 'sync_status', 'sku', 'quantity', 'sync_at', 'marketplace', 'notes']
_INSIGHT_KEY = 'sync_state'
...
def _sync_brief(payload: Dict[str, Any]) -> str:
    """The sync line an integration operator reads."""
    parts = [
        "Integration " + (_text(payload, "integration_name") or "unnamed integration"),
        "channel: " + (_text(payload, "channel") or "in_store"),
        "marketplace: " + (_text(payload, "marketplace") or "direct"),
        "external order: " + (_text(payload, "external_order_id") or "none"),
        "direction: " + (_text(payload, "sync_direction") or "inbound"),
        "sync status: " + (_text(payload, "sync_status") or "pending"),
    ]
    return "; ".join(parts)


def _sync_topic(payload: Dict[str, Any]) -> str:
    """Store bus topic for this sync; always non-empty."""
    channel = (_text(payload, "channel") or "in_store").lower().replace(" ", "_")
    return "retail.omnichannel.synced." + (channel or "in_store")


def _sync_query(payload: Dict[str, Any]) -> str:
    """The corpus query this sync searches for (never empty)."""
    parts = [
        _text(payload, "integration_name"),
        _text(payload, "channel"),
        _text(payload, "marketplace"),
        _text(payload, "sku"),
    ]
    return " ".join(part for part in parts if part) or "channel synchronisation"


def _normalise_channels(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Sync normalisation: every event names an integration and a channel."""
    data = dict(payload) if isinstance(payload, dict) else {}
    reference = _text(data, "reference") or "unreferenced"
    data["reference"] = reference
    data["integration_name"] = _text(data, "integration_name") or reference
    data["channel"] = (_text(data, "channel") or "in_store").lower()
    data["sync_direction"] = (_text(data, "sync_direction") or "inbound").lower()
    return data


def _insight(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Sync state read back off the record."""
    return {
        "integration_name": _text(payload, "integration_name"),
        "channel": _text(payload, "channel"),
        "sync_direction": _text(payload, "sync_direction"),
        "quantity": _int(payload, "quantity", 0),
        "topic": _sync_topic(payload),
    }

def _block_input(block_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """The object each bound block is handed, built from the sync record."""
    reference = _text(payload, "reference") or "unreferenced"
    name = _text(payload, "integration_name") or "unnamed integration"
    brief = _sync_brief(payload)
    query = _sync_query(payload)
    if block_id == "event_bus":
        ...
        return {
            ...
            "payload": {
                ...
                "external_order_id": _text(payload, "external_order_id"),
                "sync_status": _text(payload, "sync_status"),
            },
            "message": brief,
            "channel": "mcp",
            "tool": "event_bus",
    ...
    if block_id == "workflow":
        # EVERY event_bus child is prepared in source (step_1 here):
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload = _normalise_channels(payload)
