"""Handler for capability patient_directory.
...
ENTITY = 'patient_directory'
BLOCK_IDS = ['database', 'capture', 'validation']
...
BLOCK_DEFAULT_ACTIONS = {'database': 'query', 'capture': 'extract', 'validation': 'validate_pipeline'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'patient_name', 'patient_email', 'patient_phone', 'date_of_birth', 'primary_provider', 'notes']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record["primary_provider"] = str(record.get("primary_provider") or "")
        record["notes"] = str(record.get("notes") or "")
        # Visit history and appointments link back through this key.
        record["patient_key"] = record["patient_name"].lower().replace(" ", "-")
        results = {}
        errors = {}
