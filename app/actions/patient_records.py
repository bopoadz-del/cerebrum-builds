"""Handler for capability patient_records.
...
BLOCK_DEFAULT_ACTIONS = {'database': 'query', 'storage': 'store', 'validation': 'validate_pipeline', 'audit': 'log'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'patient_name', 'patient_email', 'patient_phone', 'date_of_birth', 'address', 'medical_history', 'dental_history', 'insurance_provider', 'insurance_member_id', 'primary_provider', 'notes']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record["patient_phone"] = str(record.get("patient_phone") or "").strip()
        record["date_of_birth"] = str(record.get("date_of_birth") or "").strip()
        record["insurance_provider"] = str(record.get("insurance_provider") or "").strip()
        record["insurance_member_id"] = str(record.get("insurance_member_id") or "").strip()
        record["primary_provider"] = str(record.get("primary_provider") or "").strip()
        record["medical_history"] = str(record.get("medical_history") or "")
        record["dental_history"] = str(record.get("dental_history") or "")
        # Front-desk search key: stable, derived from the profile it belongs to.
        # Appointment, treatment and recall rows link back through this key.
        key = "".join(ch if ch.isalnum() else "-" for ch in name.lower()).strip("-")
        record["patient_key"] = key or "patient"
        record["search_text"] = " ".join(
            part for part in (
                record["patient_name"].lower(),
                record["patient_phone"],
                record["patient_email"].lower(),
                record["primary_provider"].lower(),
            ) if part
        )
        results = {}
        errors = {}
