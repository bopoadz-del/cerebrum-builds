"""Handler for capability patient_visit_records.
...
ENTITY = 'patient_visit_records'
BLOCK_IDS = ['capture', 'database', 'validation', 'audit']
...
BLOCK_DEFAULT_ACTIONS = {'capture': 'extract', 'database': 'query', 'validation': 'validate_pipeline', 'audit': 'log'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'patient_name', 'tooth', 'procedure', 'fee', 'visit_date', 'provider', 'clinical_notes']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record["procedure"] = str(record.get("procedure") or "").strip()
        if not record["patient_name"] or not record["tooth"] or not record["procedure"]:
            return {"ok": False, "capability": CAPABILITY_ID,
                    "error": "Missing required fields: patient_name, tooth, procedure"}
        fee = record.get("fee")
        if isinstance(fee, bool) or not isinstance(fee, (int, float)):
            fee = 0.0
        # Fee arithmetic the clinician reviews before it is billed.
        record["fee"] = round(float(fee), 2)
        record["visit_date"] = record.get("visit_date") or "2026-09-03"
        record["provider"] = str(record.get("provider") or "")
        record["clinical_notes"] = str(record.get("clinical_notes") or "")
        # Tooth-level summary carried into the visit-history index.
        record["visit_summary"] = (
            "tooth " + record["tooth"] + " " + record["procedure"]
            + " fee " + ("%.2f" % record["fee"])
        )
        results = {}
        errors = {}
