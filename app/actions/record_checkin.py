"""Handler for capability record_checkin.

Written by the factory WRITER role (codewhale exec)

Records one guest check-in at the front desk: the clerk's record is
structured by the vendored ``capture`` slice, screened by the
vendored ``validation`` slice against this capability's declared
block roster, and mirrored through the vendored ``database`` slice --
then persisted to this capability's own alembic entity by the route.

Scope
-----
READS  the caller's own record (reference, guest_name, room_number, nights, arrival_time, notes, status); app.dispatch (the local offline
...
``capture``     structures the typed check-in into front-desk fields
(extract);
``validation``  confirms every block this capability declares is
available (validate_dependencies);
``database``    reads the entity surface back
(query) so the result carries the store's own count.

Blocks: capture=extract, validation=validate_dependencies, database=query
...
BLOCK_IDS = ['capture', 'validation', 'database']
...
CAPABILITY_FIELDS = ['reference', 'guest_name', 'room_number', 'nights', 'arrival_time', 'notes', 'status']
...
ROOM_NUMBER_RE = re.compile(r"^[0-9A-Za-z][0-9A-Za-z\-]{0,9}$")
TIME_RE = re.compile(r"^([01]\d|2[0-3]):[0-5]\d(:[0-5]\d)?$")
CONSTANTS = {"min_nights": 1, "max_nights": 30, "house_rooms": 40}


def _block_input(block_id, record):
    """The input each bound block actually gets. Caller supplies none of it."""
    if block_id == "capture":
        # The capture slice reads raw_text/text/content/body off its payload.
        return {"raw_text": _summary(record, "front desk check-in")}
    if block_id == "validation":
        return {"block_id": CAPABILITY_ID, "dependencies": list(BLOCK_IDS)}
    if block_id == "database":
        # The Store database slice is a plain SQL surface: a read of this
        # capability's own entity, never the caller's table choice.
        return {"table": ENTITY, "sql": f"SELECT * FROM {ENTITY}"}
    return dict(record)


def _guard(record):
    """Domain validation this capability owns (not a block contract)."""
    ...
    if isinstance(nights, bool) or not isinstance(nights, int):
        return "nights must be an integer"
    if not CONSTANTS["min_nights"] <= nights <= CONSTANTS["max_nights"]:
        return (
            f"nights must be between {CONSTANTS['min_nights']} and "
            f"{CONSTANTS['max_nights']}"
        )
    room = str(record.get("room_number") or "")
    if not ROOM_NUMBER_RE.match(room):
        return "room_number must be an alphanumeric room label"
    arrival = str(record.get("arrival_time") or "")
    if not TIME_RE.match(arrival):
        return "arrival_time must be HH:MM or HH:MM:SS"
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        captured = results.get("capture") if isinstance(results.get("capture"), dict) else {}
        ...
        return {
            ...
            "front_desk": {
                ...
                "room_label": record.get("room_number"),
                "stay_nights": record.get("nights"),
                "house_rooms": CONSTANTS["house_rooms"],
            },
            "capture": {
                "capture_id": captured.get("capture_id"),
                "engine": captured.get("ocr_engine"),
                "posture": captured.get("posture"),
            ...
            "database": {"surface_rows": (rows[0] if rows else {})},
            "results": results,
        }
