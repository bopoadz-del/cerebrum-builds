"""Handler for capability rental_contract_management.
...
ENTITY = 'rental_contract_management'
BLOCK_IDS = ['workflow', 'validation', 'event_bus', 'queue', 'audit']
#: Each block's declared default action. Blocks are action-dispatched;
#: calling one with no action is answered with an error.
BLOCK_DEFAULT_ACTIONS = {'workflow': 'run', 'validation': 'validate_pipeline', 'event_bus': 'publish', 'queue': 'enqueue', 'audit': 'log'}
#: This capability's own domain columns.
CAPABILITY_FIELDS = ['customer_name', 'vehicle_reference', 'branch', 'pickup_date', 'return_date', 'extension_days', 'daily_rate', 'deposit_amount', 'damage_notes', 'reference', 'status']
...
def _domain_record(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Rental contract row with the derived rental window and charges."""
    record = {k: v for k, v in (payload or {}).items() if v is not None}
    scheduled = _days_between(record.get("pickup_date"), record.get("return_date"))
    extension = _as_int(record.get("extension_days") or 0)
    record["rental_days"] = scheduled
    record["billed_days"] = scheduled + extension
    try:
        rate = _as_float(record.get("daily_rate") or 0)
    except (TypeError, ValueError):
        rate = 0.0
    record["rental_charge"] = round(rate * record["billed_days"], 2)
    record["extension_charge"] = round(rate * extension, 2)
    record["availability_key"] = "%s|%s|%s" % (
        record.get("vehicle_reference") or "unassigned",
        record.get("pickup_date") or "na",
        record.get("return_date") or "na",
    )
    record["contract_summary"] = (
        "%s rents %s for %d day(s) from %s (%s)"
        % (
            record.get("customer_name") or "customer",
            record.get("vehicle_reference") or "vehicle",
            record["billed_days"],
            record.get("pickup_date") or "unscheduled",
            record.get("branch") or "unassigned",
        )
    )
    return record
...
def _prepared_event_steps(record: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Every workflow child carries the prepared event_bus contract.

    The Store workflow records a child refusal as step_N (event_bus): error,
    so step_0 and every later step carry topic, a payload dict, a message and
    channel=mcp -- never the raw schema sample.
    """
    topic = "rental.contract.%s" % str(record.get("status") or "open")
    message = str(record.get("contract_summary") or "rental contract updated")
    payload = {
        "reference": record.get("reference") or "rental_contract_management",
        "vehicle_reference": record.get("vehicle_reference") or "",
        "branch": record.get("branch") or "",
        "billed_days": record.get("billed_days") or 0,
        "rental_charge": record.get("rental_charge") or 0,
    }
    return [
        {
            "block": "event_bus",
...
def _email_notice(record: Dict[str, Any]) -> Dict[str, str]:
    """The branch notification for this contract, queued by the mail relay."""
    message = str(record.get("contract_summary") or "rental contract updated")
    return {
        "to": "branch-manager@example.com",
        "role": "branch_manager",
        "subject": "Rental contract %s" % (record.get("reference") or ""),
        "body": message,
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(result, dict) and result.get('ok') is not False:
        ...
        result = dict(result)
        result['summary'] = _record.get('contract_summary') or result.get('capability')
