"""Handler for capability reporting_analytics.
...
ENTITY = 'reporting_analytics'
BLOCK_IDS = ['analytics', 'dashboard', 'memory', 'vector_search']
#: Each block's declared default action. Blocks are action-dispatched;
#: calling one with no action is answered with an error.
BLOCK_DEFAULT_ACTIONS = {'analytics': 'track_event', 'dashboard': 'render', 'memory': 'get', 'vector_search': 'search'}
#: This capability's own domain columns.
CAPABILITY_FIELDS = ['metric', 'value', 'branch', 'period', 'query', 'reference', 'status']


def _domain_record(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Analytics row: the metric reading plus its derived band."""
    ...
    value = _num("value")
    metric = str(record.get("metric") or "fleet_utilisation")
    record["metric"] = metric
    record["value"] = value
    record["reading"] = "%s=%s for %s (%s)" % (
        metric,
        value,
        record.get("branch") or "all branches",
        record.get("period") or "current",
    )
    if metric in ("fleet_utilisation", "utilisation"):
        record["band"] = "high" if value >= 75 else "medium" if value >= 50 else "low"
        record["alert"] = value < 50
    elif metric in ("unpaid_invoices", "overdue_returns", "maintenance_cost"):
        record["band"] = "high" if value > 0 else "none"
        record["alert"] = value > 0
    else:
        record["band"] = "tracked"
        record["alert"] = False
    record["query_text"] = str(
        record.get("query") or record["reading"]
    )
    return record
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(result, dict) and result.get('ok') is not False:
        ...
        result = dict(result)
        result['summary'] = _record.get('reading') or result.get('capability')
