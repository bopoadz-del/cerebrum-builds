...
BLOCK_DEFAULT_ACTIONS = {'team': 'get_team_context', 'audit': 'log'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'staff_name', 'staff_email', 'staff_role', 'permission_scope', 'active_from']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record = dict(payload)
        record["staff_name"] = str(record.get("staff_name") or "").strip()
        role = str(record.get("staff_role") or "").strip()
        if not record["staff_name"] or not role:
            return {"ok": False, "capability": CAPABILITY_ID,
                    "error": "Missing required fields: staff_name, staff_role"}
        # Front desk owns scheduling; clinicians own clinical records.
        scopes = {
            "receptionist": "appointments*scheduling",
            "dentist": "clinical_records*visit_history",
            "hygienist": "clinical_records*visit_history",
            "admin": "*",
        }
        record["permission_scope"] = record.get("permission_scope") or scopes.get(role, "")
        record["staff_email"] = str(record.get("staff_email") or "")
        record["active_from"] = record.get("active_from") or "2026-09-03"
        results = {}
        errors = {}
