...
Pipeline (block inputs are constructed in ``app/block_inputs.py``):
  ...
  * ``workflow``   — the movement run. EVERY child is prepared in source:
      - step_0 ``database``  — writes the movement row;
      - step_1 ``analytics`` — records the position metric;
      - step_2 ``event_bus`` — publishes the movement event with the full
...
def _movement_steps(record: Dict[str, Any], decision: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "action": "publish",
            "input": {
                "topic": "stock.%s" % str(record.get("movement_type") or "movement"),
                "payload": {
                    "reference": str(record.get("reference") or "sample"),
                    "shop_code": decision["shop"],
                    "item_code": decision["item"],
                    "movement_type": str(record.get("movement_type") or "movement"),
                    "reorder_required": decision["reorder_required"],
                },
                "message": "stock %s %s at %s"
                % (
                    decision["item"],
                    str(record.get("movement_type") or "movement"),
                    decision["shop"],
                ),
                "channel": "mcp",
                "tool": "event_bus",
