"""Handler for capability todays_appointment_list.
...
ENTITY = 'todays_appointment_list'
BLOCK_IDS = ['dashboard', 'database', 'workflow']
...
BLOCK_DEFAULT_ACTIONS = {'dashboard': 'render', 'database': 'query', 'workflow': 'run'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'list_date', 'patient_name', 'provider', 'procedure', 'appointment_time', 'appointment_status', 'chair_room']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record["list_date"] = str(record.get("list_date") or "").strip()
        if not record["patient_name"] or not record["list_date"]:
            return {"ok": False, "capability": CAPABILITY_ID,
                    "error": "Missing required fields: patient_name, list_date"}
        record.setdefault("appointment_status", "scheduled")
        record.setdefault("appointment_time", "10:00:00")
        # Time-ordered front-desk row: sort key is the appointment clock.
        record["sort_key"] = record["list_date"] + "T" + str(record["appointment_time"])
        record["provider"] = str(record.get("provider") or "")
        record["procedure"] = str(record.get("procedure") or "")
        ...
        steps = [{
            ...
            "action": "publish",
            "input": {
                "topic": "appointment.list.refreshed",
                "payload": {
                    "reference": payload.get("reference") or "todays_list",
                    "list_date": record["list_date"],
                    "appointment_status": record["appointment_status"],
                },
                "message": "today's appointment list refreshed for the front desk",
                "channel": "mcp",
                "tool": "event_bus",
            },
        }]
