"""Handler for capability treatment_records.
...
BLOCK_IDS = ['database', 'document_engine', 'storage', 'audit']
...
BLOCK_DEFAULT_ACTIONS = {'database': 'query', 'document_engine': 'parse', 'storage': 'store', 'audit': 'log'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'patient_name', 'tooth', 'procedure', 'provider', 'treatment_date', 'clinical_notes', 'follow_up_date', 'attachment_name']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        record = dict(payload)
        procedure = str(record.get("procedure") or "").strip()
        record["status"] = str(record.get("status") or "open")
        record["patient_name"] = str(
            record.get("patient_name") or record.get("reference") or "unknown patient"
        ).strip()
        # A clinical note without a named procedure is recorded as an
        # unclassified visit rather than refused: the front desk must be able
        # to file a visit and let the clinician backfill the code.
        record["procedure"] = procedure or "unclassified"
        record["tooth"] = str(record.get("tooth") or "").strip()
        record["provider"] = str(record.get("provider") or "").strip()
        record["treatment_date"] = str(record.get("treatment_date") or "").strip()
        record["follow_up_date"] = str(record.get("follow_up_date") or "").strip()
        record["clinical_notes"] = str(record.get("clinical_notes") or "")
        record["attachment_name"] = str(
            record.get("attachment_name") or f"{procedure.replace(' ', '_')}_note"
        )
        # The clinical note is the document document_engine parses and storage
        # files; both blocks get the same content so the record, the note and the
        # stored artifact cannot drift apart.
        record["content"] = record["clinical_notes"] or (
            f"{record['procedure']} for {record['patient_name']}"
        )
        record["filename"] = record["attachment_name"]
        results = {}
        errors = {}
