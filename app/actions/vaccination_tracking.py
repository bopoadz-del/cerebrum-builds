"""Handler for capability vaccination_tracking.
...
ENTITY = 'vaccination_tracking'
BLOCK_IDS = ['estate_registry', 'readiness_engine', 'notification', 'audit']
...
BLOCK_DEFAULT_ACTIONS = {'estate_registry': 'register', 'readiness_engine': 'score', 'notification': 'send', 'audit': 'log'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'patient_name', 'vaccine_type', 'administered_on', 'next_due_on', 'lot_number', 'administered_by', 'reminder_channel']
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _impl(payload, execute=_watched):
        ...
        vaccine = str(record.get("vaccine_type") or "").strip()
        if not record["patient_name"] or not vaccine:
            return {"ok": False, "capability": CAPABILITY_ID,
                    "error": "Missing required field: vaccine_type"}
        administered = str(record.get("administered_on") or "").strip()
        if administered and not str(record.get("next_due_on") or "").strip():
            from datetime import date, timedelta
            try:
                base = date.fromisoformat(administered[:10])
            except ValueError:
                base = None
            if base is not None:
                record["next_due_on"] = (base + timedelta(days=365)).isoformat()
