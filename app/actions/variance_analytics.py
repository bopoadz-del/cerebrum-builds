"""Handler for capability variance_analytics.
...
ENTITY = 'variance_analytics'
BLOCK_IDS = ['analytics', 'formula_executor', 'dashboard', 'portfolio_rollup', 'database']
...
BLOCK_DEFAULT_ACTIONS = {'analytics': 'track_event', 'formula_executor': 'execute', 'dashboard': 'render', 'portfolio_rollup': 'aggregate', 'database': 'query'}
...
CAPABILITY_FIELDS = ['reference', 'status', 'department', 'category', 'period', 'budget_amount', 'actual_amount', 'formula', 'notes']

_REQUIRED_FIELDS = ['reference', 'status', 'department']

def _variance(record):
    """Variance and variance-vs-budget for one department/period line."""
    ...
    out = {
        ...
        'configured_formula': str(record.get('formula') or 'variance_vs_budget'),
    }
    try:
        out['variance_amount'] = formulas.variance_amount(
            budget_amount=record.get('budget_amount'),
            actual_amount=record.get('actual_amount'))
        out['variance_percent'] = formulas.variance_percent(
            budget_amount=record.get('budget_amount'),
            actual_amount=record.get('actual_amount'))
        out['favourable'] = out['variance_amount'] <= 0
...
def _platform_side(record, unavailable):
    """The variance figure, its direction and the analytic event."""
    variance = _variance(record)
    try:
        from app import notifications

        variance['notification'] = notifications.deliver(
            department=str(record.get('department') or ''),
            subject='variance: %s %s' % (variance['department'], variance['period']),
            message='budget=%s actual=%s variance=%s (%s)' % (
                record.get('budget_amount'), record.get('actual_amount'),
                variance.get('variance_amount'), variance.get('variance_percent')),
            channel='mcp', record=record)
    except Exception as exc:  # noqa: BLE001
        variance['notification'] = {'error': '%s: %s' % (type(exc).__name__, exc)}
    return {'variance': variance, 'blocks_unavailable': sorted(unavailable)}

def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Variance and variance-vs-budget analytics across departments, categories, periods and owners."""
    ...
    # Reads the variance line; writes one result envelope. Formulas are computed, never guessed.
