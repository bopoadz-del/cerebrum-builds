...
def read_tenant(request: Request) -> Any:
    """Tenant for a read path: token when presented, the default otherwise.

    Writes keep calling require_platform_token (401 without a token); only
    the list/read surface accepts an anonymous caller, and even then it is
    scoped to the resolved tenant -- never to a caller-named one.
    """
    import app.tenancy as tenancy

    try:
        return tenancy.resolve_read_tenant(request.headers)
    except tenancy.TenantRefused:
        raise HTTPException(status_code=401, detail="authentication_required")
