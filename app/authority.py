"""precedence.v1 — authority layers for VetClinicOS answers.

Written by the factory WRITER role (codewhale exec).

Every answer this platform gives about clinical practice is assembled from
versioned data, and the winner between conflicting sources is decided here —
never by the model:

    layer 1  certified      the practice's certified clinical protocols
    layer 2  documents      the clinic's own SOPs and ingested corpus
...
DEFAULT_LADDER: Tuple[Dict[str, Any], ...] = (
    {
        ...
        "description": "Certified clinical protocols issued by the practice owner.",
    },
    {
        "layer": LAYER_DOCUMENTS,
        "id": "documents",
        "source": "clinic_corpus",
        "description": "The clinic's own SOPs, guidance and ingested documents.",
    },
    {
        "layer": LAYER_FORMULAS,
        "id": "formulas",
        "source": "app.formulas",
        "description": "Dose, tax and total calculations derived by formula.",
    },
    {
    ...
    claims = [
        {
            "text": hit.get("text"),
            "layer": LAYER_DOCUMENTS,
            "source": (hit.get("metadata") or {}).get("collection") or "clinic_corpus",
            "value": hit.get("text"),
        }
        for hit in hits
    ]
