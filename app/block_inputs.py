...
STORE_BLOCK_DEFAULT_ACTIONS = {'analytics': 'track_event', 'audit': 'log', 'capture': 'extract', 'capture_v2': 'extract', 'dashboard': 'render', 'database': 'query', 'document_engine': 'parse', 'event_bus': 'publish', 'formula_executor': 'execute', 'formula_executor_v2': 'execute', 'notification': 'send', 'queue': 'enqueue', 'team': 'create_team', 'validation': 'validate', 'vector_search': 'search', 'workflow': 'run', 'spec_analyzer': 'analyze', 'recommendation_template': 'apply_template', 'readiness_engine': 'evaluate', 'storage': 'store', 'estate_registry': 'create', 'estate_maintenance': 'create', 'evidence_verifier': 'store', 'portfolio_rollup': 'aggregate', 'knowledge': 'search'}
...
def _precondition_owner(block_id):
    """The user the precondition's ensure step created the resource for."""
    try:
        from app.preconditions import owner_user_id
    except Exception:
        return None
    try:
        owner = owner_user_id(block_id)
    except Exception:
        return None
...
def _platform_team_id():
    """The team id the platform precondition minted in THIS store root.

    ``resource_id`` is storage-root aware (see app/preconditions): a root
    change clears the cache and this function re-ensures, so the id handed
    to the ``team`` block names a team that exists in the root the block is
    reading. Handing over a stale id is what made the block answer
    "Team access denied" and took three capabilities red.
    """
    try:
        from app.preconditions import ensure_all, resource_id
    except Exception:
        return None
...
def _for_team(data: Dict[str, Any], *, product_name: str = "platform") -> Dict[str, Any]:
    ...
    if minted and (not current or not _looks_minted_team_id(current)):
        out["team_id"] = minted
        # The membership that team block mints belongs to the precondition's
        # own user: get_team_context answers "Team access denied" unless the
        # pair matches, so the id and its owner travel together.
        owner = _precondition_owner("team")
        if owner:
            out["user_id"] = owner
    for key in _TEAM_STRING_KEYS:
        if key in out and out[key] is None:
