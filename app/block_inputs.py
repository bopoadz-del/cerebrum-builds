...
#: A block refusal that means "the state you asked for already holds".
_IDEMPOTENT_REFUSALS = (
    ("team", "already exists"),
)


def idempotent_refusal(
    block_id: str, result: Any, prepared: Any = None
) -> Optional[Dict[str, Any]]:
    """Recognise an ensure-step refusal whose end state is what was asked for.

    ``team`` mints the platform's team once (app/preconditions.py). A second
    ``create_team`` for the same slug answers "Team slug ... already exists",
    which is not a capability failure: the team the step wanted is there. The
    refusal is reported as a success carrying the prepared team id.
    """
    ...
    if not text:
        return None
    bid = str(block_id or "")
    for name, needle in _IDEMPOTENT_REFUSALS:
        if bid == name and needle in text:
            carried = None
            if isinstance(prepared, dict):
                carried = prepared.get("team_id") or prepared.get("id")
            return {
                "status": "success",
                "block": bid,
...
def _minimal_pdf_bytes(text: str = "platform record") -> bytes:
    """A structurally valid single-page PDF carrying ``text``.

    document_engine parses whatever path it is handed, so the platform's
    synthesized document must actually parse: pypdf rejects a truncated
    stream outright (``PdfStreamError``), which would turn "no document
    supplied" into a block failure. Offsets are computed, not guessed.
    """
    safe = "".join(
        ch if 32 <= ord(ch) < 127 and ch not in "()\\" else " "
        for ch in str(text or "platform record")
    )[:1200]
    escaped = safe.replace("(", "").replace(")", "")
    content = ("BT /F1 9 Tf 20 180 Td 12 TL 0 -12 Td (%s) Tj ET" % escaped).encode("ascii", "replace")
    objects = [
...
def _for_file_hasher(data: Dict[str, Any], entity: Optional[str] = None) -> Dict[str, Any]:
    """Point file_hasher at a real file it can hash.

    The block answers "No file path provided" without one. The record's own
    scalar content is materialized under STORAGE_PATH/evidence and hashed —
    never a path we cannot open, and never a success we did not measure.
    """
    out = dict(data)
    for key in ("file_path", "path", "pdf_path"):
        existing = _existing_file_path(out.get(key))
        if existing:
            out["file_path"] = existing
            return out
    import hashlib as _hashlib
    from pathlib import Path as _Path

    body = json.dumps(data, default=str, sort_keys=True)
    root = _Path(os.environ.get("STORAGE_PATH") or ".") / "evidence"
    root.mkdir(parents=True, exist_ok=True)
    digest = _hashlib.sha256(body.encode("utf-8")).hexdigest()[:16]
    target = root / f"{entity or 'evidence'}-{digest}.json"
...
def prepare_block_input(
    ...
    *,
    ...
    if bid == "file_hasher":
        return _for_file_hasher(data, entity=entity)
    if bid == "dashboard":
        return _for_dashboard(data)
    if bid == "estate_registry":
        return _for_estate_registry(data)
    if bid == "estate_maintenance":
        return _for_estate_maintenance(data)
    if bid == "readiness_engine":
        return _for_readiness_engine(data)
    if bid == "portfolio_rollup":
        return _for_portfolio_rollup(data)
    if bid == "vector_search":
        return _for_vector_search(data)
    return data


_SATISFIED_WORDS = frozenset(
    {"met", "ok", "pass", "passed", "true", "yes", "y", "ready", "complete", "completed", "done", "clear"}
)


def _block_store_dir(name: str) -> str:
    """A per-platform store directory for a file-backed Store block.

    The estate blocks default to a shared system temp directory. Two
    capabilities writing the same registry ids across runs would then answer
    "duplicate record id" on the second POST. Keeping the directory under
    STORAGE_PATH scopes it to this platform's own data root.
    """
    root = Path(os.environ.get("STORAGE_PATH") or "./data")
    target = root / "block_store" / name
    target.mkdir(parents=True, exist_ok=True)
    return str(target)


def _for_estate_registry(data: Dict[str, Any]) -> Dict[str, Any]:
    """estate_registry creates by ``id``; the domain record keys off reference.

    The id this platform registers under is the record's own reference plus a
    short suffix: the registry refuses a duplicate id, and a round-trip suite
    posts the same reference more than once.
    """
    import uuid
    ...
    reference = str(
        ...
        or out.get("document_title")
        or "record"
    )
    slug = re.sub(r"[^A-Za-z0-9_.-]+", "-", reference).strip("-") or "record"
    out["id"] = f"{slug}-{uuid.uuid4().hex[:8]}"
    out.setdefault("reference", reference)
    out.setdefault("name", str(out.get("job_name") or reference))
    out.setdefault("kind", str(out.get("record_kind") or "job"))
    out.setdefault("store_dir", _block_store_dir("estate_registry"))
    return out


def _for_estate_maintenance(data: Dict[str, Any]) -> Dict[str, Any]:
    """estate_maintenance creates a work order from ``title``."""
    out = dict(data)
    title = (
        out.get("document_title")
        or out.get("job_name")
        or out.get("method_statement_ref")
        or out.get("reference")
        or "work order"
    )
    out.setdefault("title", str(title))
    out.setdefault(
        "due",
        str(out.get("milestone_due_date") or out.get("due_on") or ""),
    )
    out.setdefault("work_front", str(out.get("work_front") or ""))
    out.setdefault("store_dir", _block_store_dir("estate_maintenance"))
    return out


def _for_readiness_engine(data: Dict[str, Any]) -> Dict[str, Any]:
    """readiness_engine evaluates ``checklist`` [{id, required}] against ``state``.

    The platform's own record carries the gate's item and its observed state
    (a work front is opened only when every required item is met). A record
    that says the item is unmet makes the gate FAIL -- which is the honest
    answer and must reach the caller as an error envelope.
    """
    out = dict(data)
    item = str(
        out.get("checklist_item")
        or out.get("work_front")
        or out.get("job_code")
        or "work_front_open"
    )
    raw_state = str(out.get("checklist_state") or "met").strip().lower()
    satisfied = raw_state in _SATISFIED_WORDS
    out["checklist"] = [
        {"id": item, "required": True, "label": str(out.get("document_title") or item)}
    ]
    out["state"] = {item: bool(satisfied)}
    out.setdefault("checklist_item", item)
    out.setdefault("checklist_state", "met" if satisfied else "unmet")
    return out


def _for_portfolio_rollup(data: Dict[str, Any]) -> Dict[str, Any]:
    """portfolio_rollup aggregates [{id, value, status}].

    The job's certified value is the number the roll-up is asked for; a job
    that has not been certified yet contributes its progress percentage.
    """
    out = dict(data)
    label = str(out.get("job_code") or out.get("reference") or "record")
    raw_value = out.get("certified_value_aed")
    if raw_value in (None, ""):
        raw_value = out.get("progress_percent")
    try:
    ...
    out["properties"] = [
        {
            "id": label,
            "value": value,
            "status": str(out.get("status") or "open"),
        }
    ]
    return out


def _for_vector_search(data: Dict[str, Any]) -> Dict[str, Any]:
    """vector_search searches with ``query`` text (never the word sample)."""
    out = dict(data)
    inner = out.get("input") if isinstance(out.get("input"), dict) else {}
    for key in ("query", "question", "text"):
        if not out.get(key) and inner.get(key):
            out[key] = inner[key]
    query = (
        out.get("query")
        or out.get("question")
        or out.get("document_title")
        or out.get("document_type")
        or out.get("reference")
        or "record"
    )
    out["query"] = str(query)
    out.setdefault("collection", str(out.get("document_type") or "platform"))
    return out


#: MCP channel sends by calling a peer block in-process. Only blocks the
#: vendored runtime registry actually carries can be called that way: a peer
#: like estate_registry/readiness_engine/portfolio_rollup is vendored as an
#: adapter but is not in vendor/cerebrum/blocks/_BLOCK_DEFS, and naming it
#: answered "Block 'estate_registry' not found".
_MCP_TARGET_PREFERENCE = ("audit", "storage", "event_bus", "dashboard", "notification")


def _mcp_target(peers):
    """First peer the vendored runtime registry can actually dispatch to."""
    names = [str(p) for p in (peers or ()) if str(p).strip()]
    try:
        from vendor.cerebrum.blocks import BLOCK_REGISTRY

        available = {str(k) for k in BLOCK_REGISTRY.keys()}
    except Exception:  # noqa: BLE001 - registry absent in unit stubs
        available = set()
    for candidate in _MCP_TARGET_PREFERENCE:
        if candidate in names and (not available or candidate in available):
            return candidate
    for name in names:
        if not available or name in available:
            return name
    return _MCP_TARGET_PREFERENCE[0]
...
def _shape_workflow_steps(
    ...
    *,
    ...
    for step in steps:
        ...
        # step["params"])``: a step-level action is inert for the child
        # (measured on this checkout -- event_bus answered
        # "Unknown action: None" from a workflow child). Mirror the
        # resolved action into the step params so the child is dispatched.
        resolved = item.get("action") or _step_action(bid, default_actions)
        if resolved:
            step_params = dict(item.get("params") or {})
            step_params.setdefault("action", resolved)
            item["params"] = step_params
        shaped.append(item)
    return shaped
...
def _for_workflow(
    ...
    *,
    ...
    for block in peers[:3]:
        ...
        if action:
            step["action"] = action
            step["params"] = {**(step.get("params") or {}), "action": action}
        built.append(step)
    if not built:
...
def _synthesized_document_path(text: str = ""):
    """A parseable PDF standing in for a document this record did not upload.

    Its single line is the record's own text, so the parsed document is the
    record rather than lorem ipsum: a capability that was handed no file
    still indexes something true about what it was told.
    """
    body = str(text or "platform record").strip() or "platform record"
    key = hashlib.sha256(body.encode("utf-8", "replace")).hexdigest()[:16]
    path = os.path.join(tempfile.gettempdir(), f"platform-doc-{key}.pdf")
...
def _for_document_engine(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if existing:
        return out
    path = _synthesized_document_path(str(text or ""))
    out["file_path"] = path
    out["pdf_path"] = path
    return out
