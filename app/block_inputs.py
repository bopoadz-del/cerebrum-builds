...
def prepare_block_input(
    ...
    *,
    ...
    if bid == "storage":
        return _for_storage(data)
    return data


def _storage_root() -> str:
    """The platform's one storage root -- app.db is the one that decides."""
    from app import db as _db

    return str(_db.storage_root())


def _for_storage(data: Dict[str, Any]) -> Dict[str, Any]:
    """Pin the block's file backend under the platform's storage root.

    Vendored ``storage`` defaults to ``./data/storage`` relative to the
    process, which on a deployed container is a second root beside the
    mounted disk. The block's own ``data_dir`` input is authoritative, so
    the caller never has to know it exists.
    """
    out = dict(data)
    inner = out.get("input") if isinstance(out.get("input"), dict) else {}
    declared = out.get("data_dir") or inner.get("data_dir")
    root = Path(_storage_root())
    if declared and str(Path(str(declared)).resolve()).startswith(str(root)):
        return out
    out["data_dir"] = str(root / "storage")
    return out
