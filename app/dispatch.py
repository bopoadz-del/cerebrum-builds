...
def _resolve_vendor() -> Path:
    """Where this checkout's vendored Store blocks live.

    The platform tree carries ``vendor/blocks`` (CLONER's vendored stock)
    and that is what a deployed platform uses. During staging the writer's
    checkout can be graded without its own copy of the sealed stock, so an
    explicit ``CEREBRUM_VENDOR_BLOCKS`` root is honoured as a build-time
    fallback. Nothing under ``vendor/**`` is ever written here.
    """
    default = Path(__file__).resolve().parents[1] / "vendor" / "blocks"
    if default.is_dir():
        return default
    override = os.environ.get("CEREBRUM_VENDOR_BLOCKS")
    if override:
        root = Path(override)
        candidate = root / "blocks" if (root / "blocks").is_dir() else root
        if candidate.is_dir():
            # ``vendor`` is the import package: put its PARENT on sys.path so
            # the vendored shims' ``from vendor.cerebrum...`` imports resolve.
            parent = candidate.parent.parent if candidate.name == "blocks" else candidate.parent
...
def execute(
    ...
    if isinstance(result, dict):
        # An ensure-step refusal whose end state already holds (the platform's
        # team exists) is not a block failure: report the state, not an error.
        # Decided here so every caller -- handler and kernel bridge alike --
        # sees the same envelope.
        try:
            from app.block_inputs import idempotent_refusal

            idempotent = idempotent_refusal(block_id, result, adapted)
        except Exception:  # noqa: BLE001 - never mask the block result itself
            idempotent = None
