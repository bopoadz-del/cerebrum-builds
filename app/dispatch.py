...
def load_block(block_id: str) -> Any:
    """Load a vendored Store block instance without executing it."""
    return _instantiate(block_id)
...
def execute(block_id: str, payload: Any = None, *, action: Optional[str] = None) -> Dict[str, Any]:
    ...
    return envelope if isinstance(envelope, dict) else {"result": envelope, "ok": True}
