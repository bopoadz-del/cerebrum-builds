...
def percent_of(numerator: Any, denominator: Any) -> float:
    """``numerator / denominator`` as a percentage rounded to 2 places.

    The base is the caller's own: the campaign dashboard reports
    answered/attempted, transfers/answered and transfers/attempted through
    this one function, so a rate can never be computed two ways in two
    places. A zero base answers 0.0 rather than raising: "nothing was
    attempted" is 0%, not a division error on a dashboard.
    """
    base = _positive_int(denominator, 0)
    if not base:
        return 0.0
    return round((float(_positive_int(numerator, 0)) / float(base)) * 100.0, 2)


def conversion_rate(attempted: Any, transferred: Any) -> float:
    """Transfers per attempted call, as a percentage rounded to 2 places."""
    return percent_of(transferred, attempted)
