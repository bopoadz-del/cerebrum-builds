...
CATALOG = {'kernel': 'COLLECTOR', 'title': 'Binding surveyor', 'mandate': "Emit intake_blueprint.v1 (COLLECTOR output IS that object — vertical, capabilities with customer words + normalized id, roles/users, data sources, integrations, constraints, done_when, each field carrying the chat turn it came from). Resolve each capability's declared block ids into harvestable contracts. Name every capability with no block as an explicit gap so the WRITER authors that logic — never drop it from the plan. Consult the coding agent for a report-only endorse/mismatch review of each binding. Do not invent block ids, do not mutate the plan, and write nothing.", 'agent': 'consult', 'resolved_blocks': ['analytics', 'audit', 'dashboard', 'database', 'document_engine', 'formula_executor', 'notification', 'queue', 'storage', 'team', 'validation', 'workflow'], 'gaps': [], 'bindings': [{'capability_id': 'patient_records', 'block_ids': ['database', 'storage', 'validation', 'audit'], 'gap': False}, {'capability_id': 'appointment_scheduling', 'block_ids': ['database', 'workflow', 'queue', 'notification'], 'gap': False}, {'capability_id': 'treatment_records', 'block_ids': ['database', 'document_engine', 'storage', 'audit'], 'gap': False}, {'capability_id': 'invoicing', 'block_ids': ['database', 'formula_executor', 'document_engine', 'validation'], 'gap': False}, {'capability_id': 'recall_reminders', 'block_ids': ['workflow', 'queue', 'notification', 'analytics'], 'gap': False}, {'capability_id': 'staff_roles_permissions', 'block_ids': ['team', 'audit', 'validation'], 'gap': False}, {'capability_id': 'clinic_dashboard', 'block_ids': ['dashboard', 'analytics', 'database'], 'gap': False}], 'agent_reviews': [], 'agent_model': ''}
CAPABILITIES = [{'id': 'patient_records', 'entity': 'patient_records', 'source': 'factory kernel execute_action template', 'http': {'create': 'POST /v1/patient_records', 'list': 'GET /v1/patient_records', 'get': 'GET /v1/patient_records/{id}', 'update': 'PUT /v1/patient_records/{id}', 'delete': 'DELETE /v1/patient_records/{id}'}}, {'id': 'appointment_scheduling', 'entity': 'appointment_scheduling', 'source': 'factory kernel execute_action template', 'http': {'create': 'POST /v1/appointment_scheduling', 'list': 'GET /v1/appointment_scheduling', 'get': 'GET /v1/appointment_scheduling/{id}', 'update': 'PUT /v1/appointment_scheduling/{id}', 'delete': 'DELETE /v1/appointment_scheduling/{id}'}}, {'id': 'treatment_records', 'entity': 'treatment_records', 'source': 'factory kernel execute_action template', 'http': {'create': 'POST /v1/treatment_records', 'list': 'GET /v1/treatment_records', 'get': 'GET /v1/treatment_records/{id}', 'update': 'PUT /v1/treatment_records/{id}', 'delete': 'DELETE /v1/treatment_records/{id}'}}, {'id': 'invoicing', 'entity': 'invoicing', 'source': 'factory kernel execute_action template', 'http': {'create': 'POST /v1/invoicing', 'list': 'GET /v1/invoicing', 'get': 'GET /v1/invoicing/{id}', 'update': 'PUT /v1/invoicing/{id}', 'delete': 'DELETE /v1/invoicing/{id}'}}, {'id': 'recall_reminders', 'entity': 'recall_reminders', 'source': 'factory kernel execute_action template', 'http': {'create': 'POST /v1/recall_reminders', 'list': 'GET /v1/recall_reminders', 'get': 'GET /v1/recall_reminders/{id}', 'update': 'PUT /v1/recall_reminders/{id}', 'delete': 'DELETE /v1/recall_reminders/{id}'}}, {'id': 'staff_roles_permissions', 'entity': 'staff_roles_permissions', 'source': 'factory kernel execute_action template', 'http': {'create': 'POST /v1/staff_roles_permissions', 'list': 'GET /v1/staff_roles_permissions', 'get': 'GET /v1/staff_roles_permissions/{id}', 'update': 'PUT /v1/staff_roles_permissions/{id}', 'delete': 'DELETE /v1/staff_roles_permissions/{id}'}}, {'id': 'clinic_dashboard', 'entity': 'clinic_dashboard', 'source': 'factory kernel execute_action template', 'http': {'create': 'POST /v1/clinic_dashboard', 'list': 'GET /v1/clinic_dashboard', 'get': 'GET /v1/clinic_dashboard/{id}', 'update': 'PUT /v1/clinic_dashboard/{id}', 'delete': 'DELETE /v1/clinic_dashboard/{id}'}}]
GATES = {'kernel': 'TESTER', 'title': 'Acceptance inspector', 'mandate': "Write and run the code-phase suite against what the WRITER produced (imports, dispatch load, models, routes answer JSON, handle() returns a mapping) and bounce those failures for another writer pass. Store-backed execute-all is pilot coverage, not this gate. The harness's acceptance IS the tester — do not consult the coding agent for extra cases. Never patch app/. Never run the suite over HTTP — GET /v1/gates describes coverage only.", 'agent': 'none', 'runs_over_http': False, 'suite': [{'file': 'tests/test_smoke.py', 'covers': 'import, offline dispatch load, handle() returns a mapping', 'gated': True}, {'file': 'tests/test_smoke.py', 'covers': 'Store-backed handle() ok and nested error scan', 'marker': 'pilot', 'gated': False}, {'file': 'tests/test_models.py', 'covers': 'fixture round-trip via store.save / store.get', 'gated': True}, {'file': 'tests/test_data_lifecycle.py', 'covers': 'Alembic up/down on populated v1, restore drill, parallel writes', 'gated': True}, {'file': 'tests/test_deploy.py', 'covers': 'Fail-closed /health, correlation logs, revision rollback identity', 'gated': True}, {'file': 'tests/test_domain_acceptance.py', 'covers': 'Ten business outcomes through execute_action', 'marker': 'pilot', 'gated': False}, {'file': 'tests/test_routes.py', 'covers': 'HTTP 200 JSON for /health, kernel jobs, and each capability POST', 'gated': True}, {'file': 'tests/test_routes.py', 'covers': 'Store-backed POST accepted (ok is not False) and persisted', 'marker': 'pilot', 'gated': False}, {'file': 'tests/agent_domain_cases.py', 'covers': 'optional coding-agent domain mutations of spec payloads', 'optional': True, 'gated': False}]}
