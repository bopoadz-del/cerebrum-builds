...
CATALOG = {'kernel': 'COLLECTOR', 'title': 'Binding surveyor', 'mandate': "Emit intake_blueprint.v1 (COLLECTOR output IS that object — vertical, capabilities with customer words + normalized id, roles/users, data sources, integrations, constraints, done_when, each field carrying the chat turn it came from). Resolve each capability's declared block ids into harvestable contracts. Name every capability with no block as an explicit gap so the WRITER authors that logic — never drop it from the plan. Consult the coding agent for a report-only endorse/mismatch review of each binding. Do not invent block ids, do not mutate the plan, and write nothing.", 'agent': 'consult', 'resolved_blocks': ['dashboard', 'database', 'validation'], 'gaps': [], 'bindings': [{'capability_id': 'record_guest_check_in', 'block_ids': ['database', 'validation'], 'gap': False}, {'capability_id': 'list_todays_check_ins', 'block_ids': ['database', 'dashboard'], 'gap': False}], 'agent_reviews': [], 'agent_model': ''}
CAPABILITIES = [{'id': 'record_guest_check_in', 'entity': 'record_guest_check_in', 'source': 'kernel execute_action template', 'http': {'create': 'POST /v1/record_guest_check_in', 'list': 'GET /v1/record_guest_check_in', 'get': 'GET /v1/record_guest_check_in/{id}', 'update': 'PUT /v1/record_guest_check_in/{id}', 'delete': 'DELETE /v1/record_guest_check_in/{id}'}}, {'id': 'list_todays_check_ins', 'entity': 'list_todays_check_ins', 'source': 'kernel execute_action template', 'http': {'create': 'POST /v1/list_todays_check_ins', 'list': 'GET /v1/list_todays_check_ins', 'get': 'GET /v1/list_todays_check_ins/{id}', 'update': 'PUT /v1/list_todays_check_ins/{id}', 'delete': 'DELETE /v1/list_todays_check_ins/{id}'}}]
GATES = {'kernel': 'TESTER', 'title': 'Acceptance inspector', 'mandate': "Write and run the code-phase suite against what the WRITER produced (imports, dispatch load, models, routes answer JSON, handle() returns a mapping) and bounce those failures for another writer pass. Store-backed execute-all is pilot coverage, not this gate. The harness's acceptance IS the tester — do not consult the coding agent for extra cases. Never patch app/. Never run the suite over HTTP — GET /v1/gates describes coverage only.", 'agent': 'none', 'runs_over_http': False, 'suite': [{'file': 'tests/test_smoke.py', 'covers': 'import, offline dispatch load, handle() returns a mapping', 'gated': True}, {'file': 'tests/test_models.py', 'covers': 'sqlite round-trip via store.save / store.get', 'gated': True}, {'file': 'tests/test_routes.py', 'covers': 'HTTP 200 JSON for /health, kernel jobs, and each capability POST', 'gated': True}, {'file': 'tests/test_routes.py', 'covers': 'Store-backed POST accepted (ok is not False) and persisted', 'marker': 'pilot', 'gated': False}]}
