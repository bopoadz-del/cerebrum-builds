    ...
    async def _handler(context: ActionContext, arguments: Dict[str, Any]) -> ActionOutcome:
        ...
        def _watched(block_id, *a, **kw):
            ...
            if isinstance(res, dict) and (
                ...
                # A block whose sealed vendored source cannot be loaded is a
                # disclosed limitation, not a silent success: the handler
                # names it in unavailable_blocks and the platform's own
                # module performs that step (app.notifications for the
                # unparsable notification block). Any block that DID load
                # and answered with an error still fails the capability.
                try:
                    from app.vendor_compat import unavailable_reason

                    if unavailable_reason(str(block_id)):
                        return res
                except Exception:  # noqa: BLE001 - never mask a real failure
                    pass
