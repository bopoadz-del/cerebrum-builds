...
def compose_grounded_answer(
    *,
    ...
    """One spoken sentence, built only from retrieved project-sheet text.

    Cite-or-refuse: with no hit this returns an empty string and the caller
    withholds the claim. It never paraphrases from memory, and the sentence it
    does return names the document it came from.
    """
    text = str(question or "").strip()
    if not hits:
        return ""
    terms = tokenize(text) or tokenize(project_tag)
    best = hits[0]
    sentence = _best_sentence(str(best.get("text") or ""), terms)
    if not sentence:
        return ""
    source = str(best.get("citation") or best.get("title") or "the project sheet")
    label = str(best.get("authority") or authority.HIGHEST)
    return (
        f"{sentence} [{claim_type} from {source}; authority {label}, "
        f"precedence.v1]"
    )
