...
app = FastAPI(title="Construction Platform", lifespan=lifespan)
...
try:
    ...
    pass
# Document Q&A surface (app/rag_routes.py) — offline lexical index inside the
# platform's own sqlite file, mounted under:
#     POST /v1/rag/ingest            GET|POST /v1/rag/query
#     POST /v1/steward/rag/ingest    GET|POST /v1/steward/rag/query
try:
...
def ui_root():
    ...
    # One console, two builds: frontend/dist/index.html is what the image
    # builds from frontend/src/App.tsx, app/static/index.html is the
    # dependency-free prebuilt copy the same image falls back to.
    for candidate in (
        here.parent / 'frontend' / 'dist' / 'index.html',
        here / 'static' / 'index.html',
        here.parent / 'frontend' / 'index.html',
    ):
        if candidate.is_file():
            return FileResponse(candidate, media_type='text/html')
    return HTMLResponse("<!doctype html><html><body><h1>Construction Platform</h1></body></html>")
