...
class GoogleDrive:
    ...
    are declared on this entity by the compiled spec: the connector names them
    as the settings that turn the live path on, and the spec carries them as
    required fields. They are stored with the record so a POST built from this
    entity's own schema round-trips, and they are treated as credential
    material everywhere else -- never logged, never echoed by /health, and not
    used to dial out unless the same names are set in the process environment.
    """
    ...
    #: environment. The factory spec declares these three on the google_drive
    #: entity, so the record carries what the caller supplied and the schema
    #: sample can express it; the row is never logged and the connector still
    #: runs stubbed unless the same names are set in the process environment.
