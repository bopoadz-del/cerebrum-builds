"""Offline notification delivery for the bakery platform.
...
a branch, renders the message, and appends a durable delivery record to the
...
VALID_CHANNELS = ("mcp", "email", "inbox", "outbox", "branch-mailbox")
...
def _recipients(record: Dict[str, Any], branch: str) -> List[str]:
    who = record.get("contact_email") or record.get("mailbox") or record.get("customer_name")
    if isinstance(who, str) and who.strip():
        return [who.strip()]
    return [f"{branch or 'head-office'}@bakery.local"]
...
def deliver(
    *,
    ...
    receipt = {
        ...
        "branch": str(branch or "head-office"),
        "recipients": _recipients(record, str(branch or "")),
        "subject": str(subject or "bakery notification")[:200],
