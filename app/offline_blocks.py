...
* **notification** spools the message to ``$STORAGE_PATH/notifications.jsonl``
  ...
  left the machine.
* **knowledge** answers from the clinic's own corpus (``app.retrieval``),
...
class OfflineKnowledgeBlock:
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        self.config: Dict[str, Any] = dict(kwargs.get("config") or {})
        self.tenant = str(os.getenv("VETCLINIC_DEFAULT_TENANT") or "local")
    ...
    def process(
        ...
        try:
            ...
            top_k = 5
        retrieval.seed_clinic_guidance(self.tenant)
        ...
        answer = {
            ...
            "grounded": bool(hits),
            "offline": True,
            "collection": collection or "clinic_corpus",
