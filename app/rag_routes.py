"""RAG ingest/query over the firm's own tenant-scoped corpus.

Written by the factory WRITER role (codewhale exec).

The firm's documents (SOPs, guidance, imported notes) are ingested here and
...
def rag_ingest(payload: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    metadata = {
        "collection": str(data.get("collection") or "firm_corpus"),
...
def _query(tenant: tenancy.Tenant, query: str, top_k: int) -> Dict[str, Any]:
    retrieval.seed_firm_guidance(tenant.tenant_id)
