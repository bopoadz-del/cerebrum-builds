"""Tenant-scoped retrieval over the tenant's own uploaded finance documents.
...
def ingest(
    *,
    ...
    try:
        cur = conn.execute(
            "INSERT INTO rag_documents (tenant_id, document, document_type, department, text, embedding)"
            " VALUES (?, ?, ?, ?, ?, ?)",
            (tenant_id, document, document_type, department, body, _pack(embed_text(body))),
        )
        conn.commit()
        return {
            "id": cur.lastrowid,
...
def search(*, tenant_id: str, query: str, limit: int = 5, department: str = "") -> List[Dict[str, Any]]:
    ...
    conn = connect()
    try:
        sql = "SELECT id, document, document_type, department, text, embedding FROM rag_documents WHERE tenant_id = ?"
    ...
    for row in rows:
        body = str(row["text"])
        # Token-level overlap, never substring: 'in' is a substring of
        # 'invoice' and substring matching made a question about an absent
        # term return a priced document.
