"""Tenant-scoped retrieval over the tenant's own facility records.
...
def ingest(
    *,
    ...
    try:
        cur = conn.execute(
            "INSERT INTO rag_documents (tenant_id, document, document_type, school, text, embedding)"
            " VALUES (?, ?, ?, ?, ?, ?)",
            (tenant_id, document, document_type, school, body, _pack(embed_text(body))),
        )
        conn.commit()
        return {
            "id": cur.lastrowid,
...
def search(*, tenant_id: str, query: str, limit: int = 5, school: str = "") -> List[Dict[str, Any]]:
    ...
    conn = connect()
    try:
        sql = "SELECT id, document, document_type, school, text, embedding FROM rag_documents WHERE tenant_id = ?"
        params: List[Any] = [tenant_id]
        if school:
            sql += " AND (school = ? OR school = '')"
    ...
    for row in rows:
        body = str(row["text"])
        # Token-level overlap, never substring: 'ac' is a substring of
        # 'academic', and substring matching once made a question about an
        # unrelated term return a record.
        body_tokens = set(tokens(body))
        lexical = sum(1 for term in terms if term in body_tokens)
        if lexical == 0:
            continue
...
async def upsert_vectors(*args: Any, **kwargs: Any) -> Dict[str, Any]:
    ...
    row = ingest(
        ...
        school=str(kwargs.get("school") or ""),
    )
