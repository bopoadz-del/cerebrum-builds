...
from app import config, dispatch, domain, store, work_queue
...
def process(item_id: int, request: Request, tenant: Tenant = Depends(resolve_principal)) -> Dict[str, Any]:
    ...
    record = {
        ...
        # voice_action, not the reserved word action: the payload dict must
        # never carry a key the dispatch layer reads as its operation.
    ...
    # The queue is the other way into the edge, so it answers to the same
    # policy: the call window and the daily cap are checked here too, not
    # only on /v1/voice/originate.
    permitted = domain.dial_permitted(item_tenant, language=record["language"])
    if not record["to_number"]:
        outcome: Dict[str, Any] = {
            "ok": False,
            "error": "queued lead has no dialable number",
        }
    elif not permitted["permitted"]:
        outcome = {
            "ok": False,
            "error": permitted["reason"],
            "pacing": permitted["pacing"],
        }
    else:
        outcome = handle_for("voice_gateway").handle(record)
        placed = outcome.get("record") if isinstance(outcome, dict) else None
        if isinstance(placed, dict) and outcome.get("ok") is not False:
            stored_edge = store.save(
                "voice_gateway",
                {
                    **{
                        key: value
                        for key, value in placed.items()
                        if key in MODELS["voice_gateway"].FIELDS
                    },
                    "call_sid": outcome.get("call_sid") or record["call_sid"],
                    "tenant_id": item_tenant,
                },
                item_tenant,
            )
            lead_id_for_attempt = (lead or {}).get("id")
            if lead_id_for_attempt:
                attempts = int((lead or {}).get("attempt_count") or 0) + 1
                ceiling = int((lead or {}).get("max_attempts") or config.MAX_ATTEMPTS)
                store.save(
                    "lead_intake_and_dial_queue",
                    {
                        "id": lead_id_for_attempt,
                        "attempt_count": attempts,
                        "last_call_sid": stored_edge.get("call_sid"),
                        "queue_state": "dialed" if attempts < ceiling else "exhausted",
                        "status": "open",
                    },
                    item_tenant,
                )
