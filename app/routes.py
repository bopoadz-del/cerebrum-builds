...
# --- record_check_in (kernel execute_action template) ---


def _record_check_in_handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    try:
        from app.actions.record_check_in import handle as _handle
...
@router.post("/record_check_in")
async def record_check_in_create(payload: Dict[str, Any], request: Request) -> Dict[str, Any]:
    from app.auth import reject_invalid_payload, require_platform_token
    require_platform_token(request)
    reject_invalid_payload("record_check_in", payload)
    CAPABILITY_ID = "record_check_in"
    handle = _record_check_in_handle
    save = lambda record: store.save("record_check_in", record)
    list_all = lambda: store.list_all("record_check_in")
...
@router.get("/record_check_in")
def record_check_in_list(request: Request) -> Dict[str, Any]:
    _read_scope(request)
    ...
    allowed = set(store.COLUMNS["record_check_in"]) | CONTROLS
    ...
    try:
        return store.query("record_check_in",
...
@router.get("/record_check_in/{item_id}")
def record_check_in_get(item_id: int, request: Request) -> Dict[str, Any]:
    from app.auth import require_platform_token
    require_platform_token(request)
    record = store.get("record_check_in", item_id)
    if record is None:
        raise HTTPException(status_code=404, detail="not found")
    return record


@router.put("/record_check_in/{item_id}")
async def record_check_in_update(item_id: int, payload: Dict[str, Any], request: Request) -> Dict[str, Any]:
    from app.auth import require_platform_token
    require_platform_token(request)
    result = await perform_domain("update", "record_check_in", {**(payload or {}), 'id': item_id})
    if result.get('status') != 'success':
        return {'ok': False,
                'error': result.get('error_message') or result.get('status'),
                'result': result}
    return {'ok': True, 'capability': "record_check_in", 'result': result}


@router.delete("/record_check_in/{item_id}")
async def record_check_in_delete(item_id: int, request: Request) -> Dict[str, Any]:
    from app.auth import require_platform_token
    require_platform_token(request)
    result = await perform_domain("delete", "record_check_in", {'id': item_id})
    if result.get('status') != 'success':
        return {'ok': False,
                'error': result.get('error_message') or result.get('status'),
                'result': result}
