"""Principal authentication for the Bakery Chain Operations HTTP surface.

Written by the factory WRITER role (codewhale exec)

One bearer token (``PLATFORM_TOKEN``, default ``dev-local-token``) identifies one
principal. The principal carries the roles this platform recognises
(``operator``, ``admin``) and the tenant it belongs to; nothing in a request may
override that tenant — an ``X-Tenant`` header that disagrees with the
authenticated principal is refused rather than trusted.
...
class Principal:
    """The authenticated caller. The tenant comes from here, never a request."""
...
def require_platform_token(request: Request) -> Principal:
    """Every write is token-guarded. Reads use ``optional_principal``."""
    return authenticate(request)


def optional_principal(request: Request) -> Optional[Principal]:
    """The principal when a token was sent, else None (anonymous read)."""
    token = bearer_token(request)
    if not token:
        return None
    return authenticate(request)
