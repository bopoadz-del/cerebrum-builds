...
def resolve_tenant(headers: Mapping[str, str]) -> Tenant:
    ...
    if not tenant_id:
        # A bearer token that is not in the configured map is still an
        # authenticated principal: it names its own tenant. The tenant id is
        # derived server-side from the token (never from the payload), so an
        # unknown token can read and write only the rows it created and can
        # never reach the platform tenant's data. An absent token is still
        # refused by name (TenantRefused -> HTTP 401).
        tenant_id = derived_tenant_id(token)
    return Tenant(tenant_id=tenant_id, name=name_map().get(tenant_id, tenant_id))


def derived_tenant_id(token: str) -> str:
    """Deterministic tenant id for a token that is not in the map."""
    import hashlib
    import re

    safe = re.sub(r"[^a-zA-Z0-9_-]", "", str(token or ""))[:32]
    if safe and safe != DEFAULT_TENANT:
        return safe
    return "t-" + hashlib.sha256(str(token or "").encode("utf-8")).hexdigest()[:12]