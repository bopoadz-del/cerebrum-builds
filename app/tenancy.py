...
#: Header a caller may present to NAME the tenant it is acting for. A name
#: is only ever checked against the credential's own tenant (see
#: app.auth.read_tenant): it can narrow nothing and widen nothing.
...
def presented_token(headers: Mapping[str, str]) -> str:
    """The credential this request presented, or "" when it presented none.

    One place reads the Authorization / X-Platform-Token pair, so a caller
    cannot smuggle a tenant by using the header form the resolver forgot.
    """
...
def default_tenant() -> Tenant:
    """The platform's own tenant: the one its own token owns.

    Resolved for callers that present no principal at all — the factory's
    one-record round-trip, a migration/backup drill, an operator reading
    the store from the shell. Those callers are the platform acting as
    itself, not an anonymous visitor, so they read the default tenant
    rather than nothing.
    """
    return Tenant(tenant_id=DEFAULT_TENANT, name=name_map().get(DEFAULT_TENANT, DEFAULT_TENANT))


def named_tenant(headers: Mapping[str, str]) -> str:
    """The tenant a caller NAMED in X-Tenant, or "" when it named none."""
    return str(
        headers.get(TENANT_HEADER) or headers.get("X-Tenant") or ""
    ).strip()
