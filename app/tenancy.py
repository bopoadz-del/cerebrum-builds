"""Tenancy for LexManage: one tenant per request, always.
...
Token → tenant mapping comes from the environment:
    ...
    LEXMANAGE_TENANT_TOKENS   "token:tenant,token:tenant" for additional staff
    LEXMANAGE_TENANT_NAMES    "tenant:display name" for readable tenant names
...
def token_map() -> Dict[str, str]:
    ...
    for tok, tenant in _pairs(os.getenv("LEXMANAGE_TENANT_TOKENS") or ""):
        mapping[tok] = tenant
    return mapping


def tenant_names() -> Dict[str, str]:
    names = {DEFAULT_TENANT: "LexManage (local)"}
    for tenant, name in _pairs(os.getenv("LEXMANAGE_TENANT_NAMES") or ""):
        names[tenant] = name
    return names
...
def role_map() -> Dict[str, Tuple[str, ...]]:
    ...
    ``LEXMANAGE_ROLE_TOKENS`` is "token:role,token:role" with role in
    operator / admin (see app/security.py).
    """
    token = (os.getenv("PLATFORM_TOKEN") or "dev-local-token").strip()
    mapping: Dict[str, Tuple[str, ...]] = {token: ("admin",)}
    for tok, role in _pairs(os.getenv("LEXMANAGE_ROLE_TOKENS") or ""):
