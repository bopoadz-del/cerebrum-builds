...
def resolve_read_tenant(headers: Mapping[str, str]) -> Tenant:
    """Resolve the tenant for a READ path.

    A presented token still selects the tenant, so a cross-tenant read stays
    scoped to its principal. A read that presents *no* token resolves to
    DEFAULT_TENANT: the capability list/read surface is the documented
    anonymous read (docs/openapi.json declares no security on ``GET
    /v1/{capability}``), while every write path keeps requiring a token. An
    unbound token is still refused -- an unknown credential is not a guest.
    """
    ...
    if not token:
        return Tenant(
            ...
            name=name_map().get(DEFAULT_TENANT, DEFAULT_TENANT),
        )
