...
class TwilioEdge:
    ...
    def dial_broker_twiml(self, body: Mapping[str, Any]) -> str:
        """Dial the broker leg and whisper to it, then join the conference."""
        record = dict(body or {})
        broker = escape(str(record.get("broker_number") or ""))
        conference = escape(str(record.get("conference_name") or ""))
        whisper = escape(str(record.get("whisper_text") or ""))
        language = str(record.get("broker_language") or "en").lower()[:2]
        voice = config.TWILIO_TTS_VOICE.get(language, config.TWILIO_TTS_VOICE["en"])
        timeout = int(config.CONFERENCE_BRIDGE_TIMEOUT_SECONDS)
        return _twiml(
            f"<Dial timeout=\"{timeout}\">"
            f"<Number url=\"/v1/voice/whisper\">{broker}</Number>"
            f"</Dial>"
            f'<Say voice="{escape(voice)}">{whisper}</Say>'
