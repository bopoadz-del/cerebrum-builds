...
Booted facility_management platform: Facility Management Platform for Schools.
A facility management platform serving a Dubai schools estate of 10 schools. It gives management oversight and control of technician and cleaner teams, lets school staff and parents log complaints (or management enter them on their behalf), auto-assigns those complaints to the right field staff where possible, and tracks every complaint and job through to closure. Management sees per-school and portfolio-level dashboards across all 10 sites, with role-based access separating management, field st

Who it is for: facility management operators
Roles: operator, admin

Capabilities:
- complaints_management [REUSE]: dual-registered  blocks=['capture', 'workflow', 'queue', 'notification']
- auto_assignment [REUSE]: dual-registered  blocks=['workflow', 'queue', 'team', 'recommendation_template']
- workforce_management [REUSE]: dual-registered  blocks=['team', 'workflow', 'queue']
- management_dashboards [REUSE]: dual-registered  blocks=['dashboard', 'analytics', 'portfolio_rollup']
- reporting [REUSE]: dual-registered  blocks=['analytics', 'portfolio_rollup', 'formula_executor', 'notification']
- role_based_access [REUSE]: dual-registered  blocks=['team', 'audit', 'validation']
- erp_integration [REUSE]: dual-registered  blocks=['workflow', 'event_bus', 'validation']
- booking_system_integration [REUSE]: dual-registered  blocks=['workflow', 'event_bus', 'validation']
...
- complaints_management: REUSE ['capture', 'workflow', 'queue', 'notification'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/complaints_management.py)
- auto_assignment: REUSE ['workflow', 'queue', 'team', 'recommendation_template'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/auto_assignment.py)
- workforce_management: REUSE ['team', 'workflow', 'queue'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/workforce_management.py)
- management_dashboards: REUSE ['dashboard', 'analytics', 'portfolio_rollup'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/management_dashboards.py)
- reporting: REUSE ['analytics', 'portfolio_rollup', 'formula_executor', 'notification'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/reporting.py)
- role_based_access: REUSE ['team', 'audit', 'validation'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/role_based_access.py)
- erp_integration: REUSE ['workflow', 'event_bus', 'validation'] (verified present in Store registry; handler source factory-grounded event_bus workflow; emit app/actions/erp_integration.py)
- booking_system_integration: REUSE ['workflow', 'event_bus', 'validation'] (verified present in Store registry; handler source factory-grounded event_bus workflow; emit app/actions/booking_system_integration.py)

GAPS (you author; do not invent a block id):
- (none)

WORK ITEMS (C-BRIEF hole-fill; GENERATE gaps plus REUSE that still need handlers):
- complaints_management: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- auto_assignment: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- workforce_management: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- management_dashboards: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- reporting: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- role_based_access: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- erp_integration: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- booking_system_integration: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
...
Verified present: capture, workflow, queue, notification, workflow, queue, team, recommendation_template, team, workflow, queue, dashboard, analytics, portfolio_rollup, analytics, portfolio_rollup, formula_executor, notification, team, audit, validation, workflow, event_bus, validation, workflow, event_bus, validation
Missing:
...
Domain pack (binding fields):
{
  ...
  "core_business_workflows": [
    "one-record round-trip for complaints_management",
    "one-record round-trip for auto_assignment",
    "one-record round-trip for workforce_management",
    "one-record round-trip for management_dashboards",
    "one-record round-trip for reporting",
    "one-record round-trip for role_based_access",
    "one-record round-trip for erp_integration",
    "one-record round-trip for booking_system_integration"
  ],
  "data_sources": [
  ...
  "domain_purpose": "A facility management platform serving a Dubai schools estate of 10 schools. It gives management oversight and control of technician and cleaner teams, lets school staff and parents log complaints (or management enter them on their behalf), auto-assigns those complaints to the right field staff where possible, and tracks every complaint and job through to closure. Management sees per-school and portfolio-level dashboards across all 10 sites, with role-based access separating management, field st",
  "domain_rules": [
  ...
  "mission": "A facility management platform serving a Dubai schools estate of 10 schools. It gives management oversight and control of technician and cleaner teams, lets school staff and parents log complaints (or management enter them on their behalf), auto-assigns those complaints to the right field staff where possible, and tracks every complaint and job through to closure. Management sees per-school and portfolio-level dashboards across all 10 sites, with role-based access separating management, field st",
  "primary_users": [
