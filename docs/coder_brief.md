...
Booted hospitality platform: Front Desk Check-In Log.
A minimal front-desk tool for small hotels that lets staff record guest check-ins and view a list of today's check-ins. It serves front-desk clerks and managers who need a simple, reliable daily log without any additional property-management features.

Who it is for: hospitality operators
Roles: operator, admin

Capabilities:
- record_guest_check_in [REUSE]: dual-registered  blocks=['capture', 'database', 'validation']
- list_todays_check_ins [REUSE]: dual-registered  blocks=['database', 'dashboard']
...
- record_guest_check_in: REUSE ['capture', 'database', 'validation'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/record_guest_check_in.py)
- list_todays_check_ins: REUSE ['database', 'dashboard'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/list_todays_check_ins.py)

GAPS (you author; do not invent a block id):
- (none)

WORK ITEMS (C-BRIEF hole-fill; GENERATE gaps plus REUSE that still need handlers):
- record_guest_check_in: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- list_todays_check_ins: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
...
Verified present: capture, database, validation, database, dashboard
Missing:
...
Launching-ready full-pilot authorship floor: emit ≥2 keepable agent-written app/actions/*.py handlers (or equivalent cli_authored_ids). This brief has 2 required capabilities, so the floor is 2 (dynamic floor — not a fixed ≥5). Fewer than 2 is FACTORY_CODE_CLI_THIN_AUTHORSHIP — CODE_GREEN / pilot_ready=false, package 409. Do not treat the job as done below this floor.
...
Domain pack (binding fields):
{
  ...
  "core_business_workflows": [
    "one-record round-trip for record_guest_check_in",
    "one-record round-trip for list_todays_check_ins"
  ],
  "data_sources": [
  ...
  "domain_purpose": "A minimal front-desk tool for small hotels that lets staff record guest check-ins and view a list of today's check-ins. It serves front-desk clerks and managers who need a simple, reliable daily log without any additional property-management features.",
  "domain_rules": [
  ...
  "mission": "A minimal front-desk tool for small hotels that lets staff record guest check-ins and view a list of today's check-ins. It serves front-desk clerks and managers who need a simple, reliable daily log without any additional property-management features.",
  "primary_users": [
...
- launching-ready full-pilot authorship: ≥2 keepable agent-written app/actions/*.py handlers (or equivalent cli_authored_ids) (dynamic floor: 2 required capabilities → need ≥2, not a fixed ≥5)  [check:full_pilot_authorship]
...
- authorship below the launching-ready full-pilot floor (<2 agent-written app/actions/*.py / cli_authored_ids) (dynamic floor) — FACTORY_CODE_CLI_THIN_AUTHORSHIP
- decorative tests
