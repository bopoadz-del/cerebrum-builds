...
Booted automotive platform: Automotive Platform.
A platform for a car dealership: vehicle inventory (make, model, year, price, mileage, VIN, status new/used), browse and filter listings, vehicle detail pages, customer inquiries and leads with follow-up notes, test-drive booking, financing interest form, and a sales-team dashboard showing leads and inventory. Multi-branch support with per-branch inventory.

Who it is for: automotive operators
Roles: operator, admin

Capabilities:
- automotive_core [GENERATE]: kernel/template generation (no block ids)  (no block ids — genuine gap)
- dashboard [REUSE]: dual-registered  blocks=['dashboard']
- team [REUSE]: dual-registered  blocks=['team']
...
- team: REUSE ['team'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/team.py)
...
- automotive_core: GAP — author this logic (kernel/template generation (no block ids))

WORK ITEMS (C-BRIEF hole-fill; GENERATE gaps plus REUSE that still need handlers):
- automotive_core: GENERATE/GAP — author this logic
- dashboard: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- team: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
...
Verified present: dashboard, team, audit
Missing:
...
Launching-ready full-pilot authorship floor: emit ≥4 keepable agent-written app/actions/*.py handlers (or equivalent cli_authored_ids). This brief has 4 required capabilities, so the floor is 4 (dynamic floor — not a fixed ≥5). Fewer than 4 is FACTORY_CODE_CLI_THIN_AUTHORSHIP — CODE_GREEN / pilot_ready=false, package 409. Do not treat the job as done below this floor.
...
Domain pack (binding fields):
{
  ...
  "core_business_workflows": [
    "one-record round-trip for automotive_core",
    "one-record round-trip for dashboard",
    "one-record round-trip for team",
  ...
  "domain_purpose": "A platform for a car dealership: vehicle inventory (make, model, year, price, mileage, VIN, status new/used), browse and filter listings, vehicle detail pages, customer inquiries and leads with follow-up notes, test-drive booking, financing interest form, and a sales-team dashboard showing leads and inventory. Multi-branch support with per-branch inventory.",
  "domain_rules": [
  ...
  "mission": "A platform for a car dealership: vehicle inventory (make, model, year, price, mileage, VIN, status new/used), browse and filter listings, vehicle detail pages, customer inquiries and leads with follow-up notes, test-drive booking, financing interest form, and a sales-team dashboard showing leads and inventory. Multi-branch support with per-branch inventory.",
  "primary_users": [
...
- launching-ready full-pilot authorship: ≥4 keepable agent-written app/actions/*.py handlers (or equivalent cli_authored_ids) (dynamic floor: 4 required capabilities → need ≥4, not a fixed ≥5)  [check:full_pilot_authorship]
...
- authorship below the launching-ready full-pilot floor (<4 agent-written app/actions/*.py / cli_authored_ids) (dynamic floor) — FACTORY_CODE_CLI_THIN_AUTHORSHIP
- decorative tests
