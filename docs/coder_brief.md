...
Booted hospitality platform: Boutique Front Desk.
A lightweight front-desk platform for a 40-room boutique hotel that lets staff quickly record guest check-ins and gives the duty manager a live view of today's arrivals. It replaces paper logs and ad-hoc spreadsheets with a shared, reliable arrival board for small hotel teams.

Who it is for: hospitality operators
Roles: operator, admin

Capabilities:
- record_checkin [REUSE]: dual-registered  blocks=['capture', 'validation', 'database']
- todays_arrivals_board [REUSE]: dual-registered  blocks=['dashboard', 'database', 'analytics']
- room_availability_check [REUSE]: dual-registered  blocks=['validation', 'database', 'formula_executor']
- guest_notes_and_preferences [REUSE]: dual-registered  blocks=['knowledge', 'database', 'memory']
- checkin_notifications [REUSE]: dual-registered  blocks=['notification', 'event_bus', 'queue']
- daily_checkin_summary [REUSE]: dual-registered  blocks=['analytics', 'database', 'dashboard']
- audit_trail [REUSE]: dual-registered  blocks=['audit', 'database']
...
- record_checkin: REUSE ['capture', 'validation', 'database'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/record_checkin.py)
- todays_arrivals_board: REUSE ['dashboard', 'database', 'analytics'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/todays_arrivals_board.py)
- room_availability_check: REUSE ['validation', 'database', 'formula_executor'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/room_availability_check.py)
- guest_notes_and_preferences: REUSE ['knowledge', 'database', 'memory'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/guest_notes_and_preferences.py)
- checkin_notifications: REUSE ['notification', 'event_bus', 'queue'] (verified present in Store registry; handler source factory-grounded event_bus workflow; emit app/actions/checkin_notifications.py)
- daily_checkin_summary: REUSE ['analytics', 'database', 'dashboard'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/daily_checkin_summary.py)
- audit_trail: REUSE ['audit', 'database'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/audit_trail.py)

GAPS (you author; do not invent a block id):
- (none)

WORK ITEMS (C-BRIEF hole-fill; GENERATE gaps plus REUSE that still need handlers):
- record_checkin: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- todays_arrivals_board: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- room_availability_check: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- guest_notes_and_preferences: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- checkin_notifications: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- daily_checkin_summary: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
...
Verified present: capture, validation, database, dashboard, database, analytics, validation, database, formula_executor, knowledge, database, memory, notification, event_bus, queue, analytics, database, dashboard, audit, database
Missing:
...
Domain pack (binding fields):
{
  ...
  "core_business_workflows": [
    "one-record round-trip for record_checkin",
    "one-record round-trip for todays_arrivals_board",
    "one-record round-trip for room_availability_check",
    "one-record round-trip for guest_notes_and_preferences",
    "one-record round-trip for checkin_notifications",
    "one-record round-trip for daily_checkin_summary",
  ...
  "domain_purpose": "A lightweight front-desk platform for a 40-room boutique hotel that lets staff quickly record guest check-ins and gives the duty manager a live view of today's arrivals. It replaces paper logs and ad-hoc spreadsheets with a shared, reliable arrival board for small hotel teams.",
  "domain_rules": [
  ...
  "mission": "A lightweight front-desk platform for a 40-room boutique hotel that lets staff quickly record guest check-ins and gives the duty manager a live view of today's arrivals. It replaces paper logs and ad-hoc spreadsheets with a shared, reliable arrival board for small hotel teams.",
  "primary_users": [
