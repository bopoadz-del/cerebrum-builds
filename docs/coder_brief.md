...
Booted bakery_management platform: Bakery Chain Operations & Delivery Platform.
A management platform for a bakery chain with five retail locations, an in-house delivery fleet (10 drivers, 10 bikes, 2 cars, ~300 deliveries/day), and central management. It centralizes stock and inventory management per shop, order and delivery dispatch, driver and vehicle scheduling, and performance analytics. It also provides a knowledge layer that accepts uploaded documents (later: unit prices and operating procedures) and answers operational questions once those documents are provided.

Who it is for: bakery management operators
Roles: operator, admin

Capabilities:
- stock_inventory_management [REUSE]: dual-registered  blocks=['database', 'workflow', 'validation', 'notification', 'analytics']
- delivery_dispatch_tracking [REUSE]: dual-registered  blocks=['database', 'workflow', 'event_bus', 'queue', 'notification', 'analytics']
- document_knowledge_qa [REUSE]: dual-registered  blocks=['document_engine', 'knowledge', 'vector_search', 'storage', 'memory', 'spec_analyzer']
- fleet_cost_and_pricing [REUSE]: dual-registered  blocks=['formula_executor', 'database', 'portfolio_rollup', 'analytics', 'validation']
- management_reporting_dashboard [REUSE]: dual-registered  blocks=['dashboard', 'analytics', 'portfolio_rollup']
- user_roles_workforce [REUSE]: dual-registered  blocks=['team', 'database', 'workflow', 'audit']
- operational_procedures_readiness [REUSE]: dual-registered  blocks=['readiness_engine', 'workflow', 'knowledge', 'notification', 'validation']
- audit_evidence_trail [REUSE]: dual-registered  blocks=['audit', 'evidence_verifier', 'file_hasher', 'database', 'storage']
...
- stock_inventory_management: REUSE ['database', 'workflow', 'validation', 'notification', 'analytics'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/stock_inventory_management.py)
- delivery_dispatch_tracking: REUSE ['database', 'workflow', 'event_bus', 'queue', 'notification', 'analytics'] (verified present in Store registry; handler source factory-grounded event_bus workflow; emit app/actions/delivery_dispatch_tracking.py)
- document_knowledge_qa: REUSE ['document_engine', 'knowledge', 'vector_search', 'storage', 'memory', 'spec_analyzer'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/document_knowledge_qa.py)
- fleet_cost_and_pricing: REUSE ['formula_executor', 'database', 'portfolio_rollup', 'analytics', 'validation'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/fleet_cost_and_pricing.py)
- management_reporting_dashboard: REUSE ['dashboard', 'analytics', 'portfolio_rollup'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/management_reporting_dashboard.py)
- user_roles_workforce: REUSE ['team', 'database', 'workflow', 'audit'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/user_roles_workforce.py)
- operational_procedures_readiness: REUSE ['readiness_engine', 'workflow', 'knowledge', 'notification', 'validation'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/operational_procedures_readiness.py)
- audit_evidence_trail: REUSE ['audit', 'evidence_verifier', 'file_hasher', 'database', 'storage'] (verified present in Store registry; handler source factory-grounded persist; emit app/actions/audit_evidence_trail.py)

GAPS (you author; do not invent a block id):
- (none)

WORK ITEMS (C-BRIEF hole-fill; GENERATE gaps plus REUSE that still need handlers):
- stock_inventory_management: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- delivery_dispatch_tracking: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- document_knowledge_qa: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- fleet_cost_and_pricing: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- management_reporting_dashboard: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- user_roles_workforce: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- operational_procedures_readiness: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
- audit_evidence_trail: REUSE hole-fill — bind persist / event_bus / BLOCK_DEFAULT_ACTIONS; do not skip because inventory_gaps is empty
...
Verified present: database, workflow, validation, notification, analytics, database, workflow, event_bus, queue, notification, analytics, document_engine, knowledge, vector_search, storage, memory, spec_analyzer, formula_executor, database, portfolio_rollup, analytics, validation, dashboard, analytics, portfolio_rollup, team, database, workflow, audit, readiness_engine, workflow, knowledge, notification, validation, audit, evidence_verifier, file_hasher, database, storage
Missing:
...
Domain pack (binding fields):
{
  ...
  "core_business_workflows": [
    "one-record round-trip for stock_inventory_management",
    "one-record round-trip for delivery_dispatch_tracking",
    "one-record round-trip for document_knowledge_qa",
    "one-record round-trip for fleet_cost_and_pricing",
    "one-record round-trip for management_reporting_dashboard",
    "one-record round-trip for user_roles_workforce",
    "one-record round-trip for operational_procedures_readiness",
    "one-record round-trip for audit_evidence_trail"
  ],
  "data_sources": [
  ...
  "domain_purpose": "A management platform for a bakery chain with five retail locations, an in-house delivery fleet (10 drivers, 10 bikes, 2 cars, ~300 deliveries/day), and central management. It centralizes stock and inventory management per shop, order and delivery dispatch, driver and vehicle scheduling, and performance analytics. It also provides a knowledge layer that accepts uploaded documents (later: unit prices and operating procedures) and answers operational questions once those documents are provided.",
  "domain_rules": [
  ...
  "mission": "A management platform for a bakery chain with five retail locations, an in-house delivery fleet (10 drivers, 10 bikes, 2 cars, ~300 deliveries/day), and central management. It centralizes stock and inventory management per shop, order and delivery dispatch, driver and vehicle scheduling, and performance analytics. It also provides a knowledge layer that accepts uploaded documents (later: unit prices and operating procedures) and answers operational questions once those documents are provided.",
  "primary_users": [
    "bakery management operators"
  ],
