...
Booted finance_ops platform: Finance Ops Platform.
Build Cerebrum FinanceOps: FP&A foundation for a mid-market finance team — chart of accounts, budget vs actual, cash forecast, close checklist, and a governed document intake for board packs. Kit-backed finance_ops. Pilot-ready platform with tests.

Who it is for: finance ops operators
Roles: operator, admin

Capabilities:
- finance_ops_core [GENERATE]: kernel/template generation (no block ids)  (no block ids — genuine gap)
...
- finance_ops_core: GAP — author this logic (kernel/template generation (no block ids))

WORK ITEMS (C-BRIEF hole-fill; GENERATE gaps plus REUSE that still need handlers):
- finance_ops_core: GENERATE/GAP — author this logic
...
Launching-ready full-pilot authorship floor: emit ≥3 keepable agent-written app/actions/*.py handlers (or equivalent cli_authored_ids). This brief has 3 required capabilities, so the floor is 3 (dynamic floor — not a fixed ≥5). Fewer than 3 is FACTORY_CODE_CLI_THIN_AUTHORSHIP — CODE_GREEN / pilot_ready=false, package 409. Do not treat the job as done below this floor.
...
Domain pack (binding fields):
{
  ...
  "core_business_workflows": [
    "one-record round-trip for finance_ops_core",
  ...
  "domain_purpose": "Build Cerebrum FinanceOps: FP&A foundation for a mid-market finance team \u2014 chart of accounts, budget vs actual, cash forecast, close checklist, and a governed document intake for board packs. Kit-backed finance_ops. Pilot-ready platform with tests.",
  "domain_rules": [
  ...
  "mission": "Build Cerebrum FinanceOps: FP&A foundation for a mid-market finance team \u2014 chart of accounts, budget vs actual, cash forecast, close checklist, and a governed document intake for board packs. Kit-backed finance_ops. Pilot-ready platform with tests.",
  "primary_users": [
    "finance ops operators"
  ],
...
- launching-ready full-pilot authorship: ≥3 keepable agent-written app/actions/*.py handlers (or equivalent cli_authored_ids) (dynamic floor: 3 required capabilities → need ≥3, not a fixed ≥5)  [check:full_pilot_authorship]
...
- authorship below the launching-ready full-pilot floor (<3 agent-written app/actions/*.py / cli_authored_ids) (dynamic floor) — FACTORY_CODE_CLI_THIN_AUTHORSHIP
- decorative tests
