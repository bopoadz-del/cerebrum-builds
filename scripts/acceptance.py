...
def check_authorship_floor() -> Tuple[str, str]:
    # A product image carries no factory package on the path: the platform
    # judges its own authorship from docs/build_provenance.json (written by
    # the factory WRITER), and falls back to counting the action modules
    # stamped "coding agent" when the helper is not importable. The import
    # must not be allowed to fail the check on its own.
    try:
        from app.factory.build.authorship import (  # type: ignore
            full_pilot_authorship_from,
        )
    except Exception:  # noqa: BLE001 - use the in-tree count below
        full_pilot_authorship_from = None
    ...
    for rel in ("docs/coder_receipt.json", "docs/build_provenance.json"):
        ...
        if path.is_file():
            try:
                ...
                pass
    try:
        if full_pilot_authorship_from is None:
            raise RuntimeError("factory authorship helper not on this path")
