...
def main() -> int:
    ...
    with TestClient(app) as client:
        ...
        missing = client.post("/v1/finance_ops_core", json={"status": "open"})
        results.append(
            _line(
        ...
        bad_enum = client.post(
            "/v1/finance_ops_core", json={"reference": "sample", "status": "bogus"}
        )
        results.append(
            _line(
        ...
        results.append(
            _line(
                "PASS" if ui.status_code == 200 and "Finance Ops Platform" in ui.text else "FAIL",
                "ui_served_200",
        ...
        ingest = client.post(
            ...
            json={
                "layer": 1,
                "doc_id": "acc-pack",
                "title": "Board pack close",
                "text": "Board pack close checklist and cash forecast for Finance Ops.",
            },
        )
        query = client.get("/v1/rag/query", params={"q": "board pack close"})
        hit = (
    ...
    if receipt_path.is_file():
        ...
        match = listed == authored and len(authored) >= 3
        results.append(
            _line(
    ...
    results.append(
        _line(
            "PASS" if len(REQUIRED_CAPABILITY_IDS) == 3 else "FAIL",
            "capability_roster",
