    ...
    with TestClient(app) as client:
        ...
        ingest = client.post(
            ...
            json={
                "layer": 1,
                "doc_id": "acc-pack",
                "title": "Operator notes pack",
                "text": "Operator notes keyword search, title and body save, and delete a note for the productivity pilot.",
            },
            headers=AUTH_HEADERS,
        )
        query = client.get("/v1/rag/query", params={"q": "notes keyword"})
        hit = (
        ...
        open_ingest = client.post(
            "/v1/rag/ingest",
            json={"layer": 1, "doc_id": "denied", "title": "x", "text": "notes keyword"},
        )
        results.append(
            _line(
        ...
        results.append(
            _line(
                ...
                "core_word_count_computed",
                f"word_count={rec.get('word_count')} title={rec.get('title')} actor={rec.get('actor')}",
            )
        )
