...
def main() -> int:
    ...
    with TestClient(app) as client:
        ...
        results.append(
            _line(
                "PASS"
                if ui.status_code == 200 and "Productivity Platform" in ui.text
                else "FAIL",
                "ui_served_200",
        ...
        ingest = client.post(
            ...
            json={
                "layer": 1,
                "doc_id": "acc-pack",
                "title": "Inbox triage pack",
                "text": "Inbox triage, keyword search, and note body snippets for the productivity pilot.",
            },
            headers=AUTH_HEADERS,
        )
        query = client.get("/v1/rag/query", params={"q": "inbox triage"})
        hit = (
        ...
        open_post = client.post(
            f"/v1/{CORE}",
            json={"reference": "sample", "status": "open", "title": "sample"},
        )
        results.append(
            _line(
        ...
        open_ingest = client.post(
            "/v1/rag/ingest",
            json={"layer": 1, "doc_id": "denied", "title": "x", "text": "inbox triage"},
        )
        results.append(
            _line(
        ...
        computed = (
            ...
            and rec.get("title") == "sample"
            and rec.get("word_count") == 1
        ...
        results.append(
            _line(
                ...
                "core_note_computed",
                f"title={rec.get('title')} word_count={rec.get('word_count')} actor={rec.get('actor')}",
            )
        )
    ...
    if receipt_path.is_file():
        ...
        match = listed == authored and len(authored) >= 2
        results.append(
            _line(
    ...
    results.append(
        _line(
            "PASS" if len(REQUIRED_CAPABILITY_IDS) == 2 else "FAIL",
            "capability_roster",
            f"{len(REQUIRED_CAPABILITY_IDS)} capabilities",
        )
    )
    render = (ROOT / "Dockerfile").is_file() and (ROOT / "render.yaml").is_file()
    results.append(_line("PASS" if render else "FAIL", "render_ready", "Dockerfile + render.yaml"))
