...
def main() -> int:
    ...
    with TestClient(app) as client:
        ...
        missing = client.post("/v1/estate_registry", json={"status": "open"})
        results.append(
            _line(
        ...
        bad_enum = client.post(
            "/v1/estate_registry", json={"reference": "sample", "status": "bogus"}
        )
        results.append(
            _line(
        ...
        results.append(
            _line(
                "PASS" if ui.status_code == 200 and "Cerebrum Steward" in ui.text else "FAIL",
                "ui_served_200",
        ...
        ingest = client.post(
            ...
            json={
                "layer": 1,
                "doc_id": "acc-sop",
                "title": "House Manual arrival",
                "text": "Guest arrival SOP for Villa Aurora house manual.",
            },
        )
        query = client.get("/v1/rag/query", params={"q": "arrival SOP Villa"})
        hit = (
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        reset_connection()
        # fail-closed: health must not claim ok when the root cannot be created as a file
    ...
    results.append(
        _line(
            "PASS" if openapi.is_file() else "FAIL",
            "openapi_committed",
            str(openapi),
        )
    )
    ...
    results.append(
        _line(
            "PASS" if len(REQUIRED_CAPABILITY_IDS) == 16 else "FAIL",
            "capability_roster",
