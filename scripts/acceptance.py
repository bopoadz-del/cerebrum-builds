...
CHECKS = ['no_token_401', 'missing_field_422', 'enum_422', 'ui_served_200', 'rag_roundtrip_hit', 'single_persistence_root', 'ci_present_and_full_suite', 'handler_bodies_distinct', 'health_fail_closed', 'openapi_committed', 'docker_health_200', 'cross_tenant_404', 'authorship_floor']
REQUIRED = 13
...
def _sample_payload(cap_id: str) -> Dict[str, Any]:
    """A payload this capability's own schema accepts.

    The earlier builder stamped the literal ``"sample"`` on every required
    field, which is not a value the spec can express for a declared int, a
    ``time``, or a ``date``: the first capability refused it (HTTP 200,
    ``ok: false``) and this check could not measure tenant isolation at all
    -- it reported "tenant A create returned no stored id" instead of
    proving a cross-tenant read is 404. The sample now follows the declared
    type and constraints, exactly like the factory's own schema sampler:
    ``allowed_values[0]`` for enums, ``min`` (else 1) for numbers, and the
    declared ``format`` for date/time/datetime. A check that cannot build a
    valid record proves nothing, and the negative control below still has to
    hold: tenant B must not see tenant A's row.
    """
    payload: Dict[str, Any] = {}
    cls = _models().get(cap_id)
    if cls is None:
        return payload
    constraints = getattr(cls, "CONSTRAINTS", {}) or {}
    try:
    ...
    for field in getattr(cls, "FIELDS", []) or []:
        ...
        if allowed:
            ...
            continue
        dtype = hints.get(field)
        if dtype is bool:
            payload[field] = False
            continue
        if dtype in (int, float):
            low = rules.get("min")
            payload[field] = low if isinstance(low, (int, float)) else (1 if dtype is int else 1.0)
            continue
        fmt = str(rules.get("format") or "").lower()
        if fmt == "time" or field.endswith("_time"):
            payload[field] = "10:00:00"
            continue
        if fmt == "datetime" or field.endswith("_at") or field.endswith("_datetime"):
            payload[field] = "2026-09-03T10:00:00"
            continue
        if fmt == "date" or field.endswith("_date"):
            payload[field] = "2026-09-03"
            continue
...
def _has_rag_surface() -> bool:
    ...
    # app/rag_routes.py is where this platform MOUNTS the surface (main.py
    # includes that router); scanning only main/routes made a shipped
    # ingest/query surface read as "no-rag-surface" and the check SKIP.
    rag_routes = ROOT / "app" / "rag_routes.py"
    text = ""
    for path in (main, routes, rag_routes):
        if path.is_file():
...
def check_cross_tenant_404(http: _Http) -> Tuple[str, str]:
    ...
    try:
        ...
        if not cap:
            return "FAIL", "no first capability to POST"
        payload: Dict[str, Any] = _sample_payload(cap)
