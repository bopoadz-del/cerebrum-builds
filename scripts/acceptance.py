...
def check_authorship_floor() -> Tuple[str, str]:
    ...
    # factory. The factory writes docs/coder_receipt.json (CLI status only),
    # so a receipt with no authorship keys is not a measured zero: fall back
    # to the disk-level stamp count in app/actions/*.py and the blueprint's
    # own n_required.
    receipt = {}
    ...
    if n_required is None:
        blueprint = ROOT / "docs" / "blueprint" / "product_blueprint.json"
        if blueprint.is_file():
            try:
                data = json.loads(blueprint.read_text(encoding="utf-8"))
                n_required = data.get("n_required")
                if n_required is None:
                    caps = [c for c in (data.get("capabilities") or []) if isinstance(c, dict)]
                    n_required = sum(1 for c in caps if c.get("required", True)) or len(caps)
            except (OSError, ValueError):
                n_required = None
    try:
        n_required_int = int(n_required) if n_required is not None else None
    except (TypeError, ValueError):
        n_required_int = None
    try:
        floor = full_pilot_authorship_from(receipt, ROOT, n_required=n_required_int)
        ...
        pass
    stamped = list(agent_written_handler_ids_in_workspace(ROOT))
    need = 5 if n_required_int is None else min(5, max(1, n_required_int))
    if len(stamped) >= need:
        return "PASS", "stamped=%s need≥%s (%s)" % (
            len(stamped),
            need,
            ", ".join(stamped),
        )
    return "FAIL", "stamped=%s below need≥%s" % (len(stamped), need)
