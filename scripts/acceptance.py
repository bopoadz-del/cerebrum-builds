    ...
    with TestClient(app) as client:
        ...
        ingest = client.post(
            ...
            json={
                ...
                "text": "Vehicle inventory, new and used listings, VIN, mileage, and sales-lead notes for the automotive pilot.",
            },
            headers=AUTH_HEADERS,
        )
        ...
        open_post = client.post(
            f"/v1/{CORE}",
            json={"reference": "sample", "status": "open", "condition": "new"},
        )
        results.append(
            _line(
        ...
        score = rec.get("inventory_score")
        computed = (
            quoted.status_code == 200
            and score not in (None, 1, 1.0)
            and rec.get("days_on_lot") == 12
            and rec.get("actor") == "operator"
        )
        results.append(
            _line(
                "PASS" if computed else "FAIL",
                "core_inventory_score_computed",
                f"inventory_score={score} days={rec.get('days_on_lot')} actor={rec.get('actor')}",
            )
        )
