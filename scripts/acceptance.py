...
def check_health_fail_closed() -> Tuple[str, str]:
    """Measure fail-closed with the disk absent, then put the env back.

    The restore is not cosmetic. ``docker_health_200`` measures GET /health
    later through this same in-process client; an unrestored STORAGE_PATH
    would point the probe at a directory that does not exist and report a
    healthy container as 503. Measure, then restore -- a check must not
    leave the process it is measuring in a state it created.
    """
