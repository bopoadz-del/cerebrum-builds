    ...
    with TestClient(app) as client:
        ...
        ingest = client.post(
            ...
            json={
                "layer": 1,
                "doc_id": "acc-pack",
                "title": "Notes inbox pack",
                "text": "Operator notes inbox keyword search and delete for the productivity pilot.",
            },
            headers=AUTH_HEADERS,
        )
        query = client.get("/v1/rag/query", params={"q": "notes inbox"})
        hit = (
        ...
        open_ingest = client.post(
            "/v1/rag/ingest",
            json={"layer": 1, "doc_id": "denied", "title": "x", "text": "notes inbox"},
        )
        results.append(
            _line(
        ...
        computed = (
            ...
            and words not in (None, 1, 1.0)
            and rec.get("note_kind") == "inbox"
        ...
        results.append(
            _line(
                ...
                f"word_count={words} kind={rec.get('note_kind')} actor={rec.get('actor')}",
            )
        )
