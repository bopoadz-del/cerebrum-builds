#!/usr/bin/env python3
"""Release gate for FleetOps Back-Office Platform.
...
def main() -> int:
    print("== FleetOps Back-Office Platform — release gate ==")
    try:
    ...
    vendored = (ROOT / "vendor" / "blocks").is_dir()
    if lock.is_file():
        ...
        # Carrying vendored blocks with no lock is untrustworthy.
        print("blocks.lock.json: MISSING — provenance of vendored blocks is unknown")
        ok = False
    else:
        print("blocks.lock.json: not present in this checkout (CLONER plants it "
              "with the vendored block tree)")
    ...
    if manifest.is_file():
        ...
        print("docs/build_provenance.json: MISSING — authorship provenance unknown")
        ok = False
