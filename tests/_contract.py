...
that contract: the previous pass hand-wrote ``{'reference': 'sample', 'status': 'open'}``
against models that also require fields such as ``job_code`` /
``document_title`` / ``subject`` and reported the product's own 422 as a
product defect.
