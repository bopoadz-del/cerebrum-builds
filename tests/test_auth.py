...
def test_mutating_route_without_token_is_401(anon_client: TestClient) -> None:
    response = anon_client.post("/v1/aircraft_maintenance_tracking", json=SCHEMA_SAMPLE)
...
def test_unconfigured_secret_fail_closed(
    ...
    with TestClient(app) as client:
        denied = client.post("/v1/aircraft_maintenance_tracking", json=SCHEMA_SAMPLE)
        assert denied.status_code == 401
...
def test_rag_ingest_without_token_is_401(anon_client: TestClient) -> None:
    response = anon_client.post(
        "/v1/rag/ingest",
        json={"layer": 1, "doc_id": "x", "title": "t", "text": "maintenance airworthiness"},
    )
    assert response.status_code == 401
...
def test_operator_token_attributes_audit(client: TestClient) -> None:
    response = client.post("/v1/aircraft_maintenance_tracking", json=SCHEMA_SAMPLE)
