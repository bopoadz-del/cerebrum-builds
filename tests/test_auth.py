...
def test_mutating_route_without_token_is_401(anon_client: TestClient) -> None:
    response = anon_client.post("/v1/inventory_stock_tracking", json=SCHEMA_SAMPLE)
...
def test_unconfigured_secret_fail_closed(
    ...
    with TestClient(app) as client:
        denied = client.post("/v1/inventory_stock_tracking", json=SCHEMA_SAMPLE)
        assert denied.status_code == 401
...
def test_rag_ingest_without_token_is_401(anon_client: TestClient) -> None:
    response = anon_client.post(
        "/v1/rag/ingest",
        json={"layer": 1, "doc_id": "x", "title": "t", "text": "low stock threshold"},
    )
    assert response.status_code == 401
...
def test_operator_token_attributes_audit(client: TestClient) -> None:
    response = client.post("/v1/inventory_stock_tracking", json=SCHEMA_SAMPLE)
    assert response.status_code == 200
    body = response.json()
    assert body.get("ok") is not False
    assert body.get("actor") == "operator"
    assert body.get("actor_role") == "operator"
    stored = client.get("/v1/inventory_stock_tracking").json().get("records") or []
    assert any(row.get("actor") == "operator" for row in stored)
