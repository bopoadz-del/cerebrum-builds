...
def test_every_model_round_trips():
    ...
    saved = store.save('record_guest_check_in', record)
    assert saved['id'] is not None, 'no id assigned for record_guest_check_in'
    fetched = store.get('record_guest_check_in', saved['id'])
    assert fetched is not None, 'record_guest_check_in did not persist'
    for key, value in record.items():
        assert fetched[key] == value, (key, fetched[key], value)
    assert any(r['id'] == saved['id'] for r in store.list_all('record_guest_check_in'))
