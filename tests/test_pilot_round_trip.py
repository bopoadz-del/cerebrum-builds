...
def test_rag_ingest_query_round_trip():
    ...
    text = (
        "LexManage conflict policy: a matter may not be opened for a client "
        "whose adverse party is already represented by the firm."
    )
    ingested = CLIENT.post(
        "/v1/rag/ingest",
        json={"text": text, "doc_id": "conflict-policy", "collection": "firm_corpus"},
        headers=AUTH,
    )
    ...
    queried = CLIENT.get(
        "/v1/rag/query", params={"q": "conflict adverse party"}, headers=AUTH
    )
...
def test_unknown_query_field_is_refused_not_ignored():
    resp = CLIENT.get("/v1/matter_management", params={"staus": "open"})
    assert resp.status_code == 200
