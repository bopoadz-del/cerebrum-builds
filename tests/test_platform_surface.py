...
def test_write_requires_a_token(client):
    assert client.post("/v1/branch_books_and_accounting", json={}).status_code == 401


def test_unknown_enum_is_refused(client):
    cls = MODELS["branch_books_and_accounting"]
    payload = _payload(cls)
    payload["status"] = "not-a-status"
    assert client.post("/v1/branch_books_and_accounting", json=payload, headers=AUTH).status_code == 422


def test_missing_required_field_is_refused(client):
    assert client.post("/v1/branch_books_and_accounting", json={}, headers=AUTH).status_code == 422


def test_unknown_record_is_404(client):
    assert client.get("/v1/order_follow_up/999999", headers=AUTH).status_code == 404
