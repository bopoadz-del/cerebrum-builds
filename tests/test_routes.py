...
def test_every_capability_route_answers():
    ...
    payload = {}
    resp = client.post("/v1/fleet_cost_and_pricing", json=payload, headers=AUTH)
    if resp.status_code != 200:
        failures.append('fleet_cost_and_pricing: HTTP ' + str(resp.status_code) + ': ' + resp.text[:200])
    else:
        try:
            body = resp.json()
        except Exception:
            failures.append('fleet_cost_and_pricing: response is not JSON')
        else:
            if not isinstance(body, dict):
                failures.append('fleet_cost_and_pricing: JSON body is not a dict')
            listed = client.get("/v1/fleet_cost_and_pricing", headers=AUTH)
            if listed.status_code != 200:
                failures.append('fleet_cost_and_pricing list: HTTP ' + str(listed.status_code))
    ...
    payload = {}
    resp = client.post("/v1/operational_procedures_readiness", json=payload, headers=AUTH)
    if resp.status_code != 200:
        failures.append('operational_procedures_readiness: HTTP ' + str(resp.status_code) + ': ' + resp.text[:200])
    else:
        try:
            body = resp.json()
        except Exception:
            failures.append('operational_procedures_readiness: response is not JSON')
        else:
            if not isinstance(body, dict):
                failures.append('operational_procedures_readiness: JSON body is not a dict')
            listed = client.get("/v1/operational_procedures_readiness", headers=AUTH)
            if listed.status_code != 200:
                failures.append('operational_procedures_readiness list: HTTP ' + str(listed.status_code))

    payload = {}
    resp = client.post("/v1/audit_evidence_trail", json=payload, headers=AUTH)
    if resp.status_code != 200:
        failures.append('audit_evidence_trail: HTTP ' + str(resp.status_code) + ': ' + resp.text[:200])
    else:
        try:
            body = resp.json()
        except Exception:
            failures.append('audit_evidence_trail: response is not JSON')
        else:
            if not isinstance(body, dict):
                failures.append('audit_evidence_trail: JSON body is not a dict')
            listed = client.get("/v1/audit_evidence_trail", headers=AUTH)
            if listed.status_code != 200:
                failures.append('audit_evidence_trail list: HTTP ' + str(listed.status_code))
...
def test_every_capability_route_accepts_payload():
    ...
    if resp.status_code != 200:
        ...
        failures.append('fleet_cost_and_pricing rejected a payload built from its own schema: ' + str(resp.json().get('error')))
    else:
        listed = client.get("/v1/fleet_cost_and_pricing", headers=AUTH)
        listed_body = listed.json() if listed.content else {}
        if listed.status_code != 200:
            ...
            failures.append('fleet_cost_and_pricing list refused: ' + str(listed_body.get('error') or listed_body)[:200])
        else:
            rows = _listed(listed_body)
            if not rows:
                failures.append('fleet_cost_and_pricing accepted a record but persisted nothing')
            else:
                item_id = rows[0].get("id")
                got = client.get(f"/v1/fleet_cost_and_pricing/{item_id}", headers=AUTH)
                if got.status_code != 200:
                    failures.append('fleet_cost_and_pricing get: HTTP ' + str(got.status_code))
                missing = client.get("/v1/fleet_cost_and_pricing/999999", headers=AUTH)
                if missing.status_code != 404:
                    failures.append('fleet_cost_and_pricing missing id: HTTP ' + str(missing.status_code) + ' (expected 404)')
    ...
    if resp.status_code != 200:
        ...
        failures.append('operational_procedures_readiness rejected a payload built from its own schema: ' + str(resp.json().get('error')))
    else:
        listed = client.get("/v1/operational_procedures_readiness", headers=AUTH)
        listed_body = listed.json() if listed.content else {}
        if listed.status_code != 200:
            ...
            failures.append('operational_procedures_readiness list refused: ' + str(listed_body.get('error') or listed_body)[:200])
        else:
            rows = _listed(listed_body)
            if not rows:
                failures.append('operational_procedures_readiness accepted a record but persisted nothing')
            else:
                item_id = rows[0].get("id")
                got = client.get(f"/v1/operational_procedures_readiness/{item_id}", headers=AUTH)
                if got.status_code != 200:
                    failures.append('operational_procedures_readiness get: HTTP ' + str(got.status_code))
                missing = client.get("/v1/operational_procedures_readiness/999999", headers=AUTH)
                if missing.status_code != 404:
                    failures.append('operational_procedures_readiness missing id: HTTP ' + str(missing.status_code) + ' (expected 404)')
    ...
    if resp.status_code != 200:
        ...
        failures.append('audit_evidence_trail rejected a payload built from its own schema: ' + str(resp.json().get('error')))
    else:
        listed = client.get("/v1/audit_evidence_trail", headers=AUTH)
        listed_body = listed.json() if listed.content else {}
        if listed.status_code != 200:
            ...
            failures.append('audit_evidence_trail list refused: ' + str(listed_body.get('error') or listed_body)[:200])
        else:
            rows = _listed(listed_body)
            if not rows:
                failures.append('audit_evidence_trail accepted a record but persisted nothing')
            else:
                item_id = rows[0].get("id")
                got = client.get(f"/v1/audit_evidence_trail/{item_id}", headers=AUTH)
                if got.status_code != 200:
                    failures.append('audit_evidence_trail get: HTTP ' + str(got.status_code))
                missing = client.get("/v1/audit_evidence_trail/999999", headers=AUTH)
                if missing.status_code != 404:
                    failures.append('audit_evidence_trail missing id: HTTP ' + str(missing.status_code) + ' (expected 404)')
