...
def test_health_and_ui(client: TestClient) -> None:
    ...
    assert "LedgerFlow" in ui.text
    assert "OPERATOR_TOKEN" in ui.text


def test_transaction_actor_on_schema_sample(client: TestClient) -> None:
    payload = _sample_payload("transaction_capture")
    response = client.post("/v1/transaction_capture", json=payload)
    assert response.status_code == 200
    body = response.json()
    assert body.get("ok") is not False
    assert body.get("actor") == "operator"
    entry = (body.get("record") or {}).get("ledger_entry") or {}
    assert entry.get("captured") is True
    assert entry.get("category") == "income"
