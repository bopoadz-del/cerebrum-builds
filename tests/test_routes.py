...
def test_health_and_ui(client: TestClient) -> None:
    ...
    assert "Aviation Operations Hub" in ui.text
    assert "OPERATOR_TOKEN" in ui.text


def test_maintenance_kernel_and_actor_on_schema_sample(client: TestClient) -> None:
    payload = _sample_payload("aircraft_maintenance_tracking")
    response = client.post("/v1/aircraft_maintenance_tracking", json=payload)
    assert response.status_code == 200
    body = response.json()
    assert body.get("ok") is not False
    assert body.get("actor") == "operator"
    mx = (body.get("record") or {}).get("maintenance") or {}
    assert mx.get("tail_number") == "sample"
    assert mx.get("kind") == "scheduled"
    assert mx.get("airworthy") is True
