...
def test_every_capability_route_answers():
    ...
    resp = client.post("/v1/record_check_in", json=payload, headers=AUTH)
    if resp.status_code != 200:
        failures.append('record_check_in: HTTP ' + str(resp.status_code) + ': ' + resp.text[:200])
    else:
        try:
            body = resp.json()
        except Exception:
            failures.append('record_check_in: response is not JSON')
        else:
            if not isinstance(body, dict):
                failures.append('record_check_in: JSON body is not a dict')
            listed = client.get("/v1/record_check_in", headers=AUTH)
            if listed.status_code != 200:
                failures.append('record_check_in list: HTTP ' + str(listed.status_code))
...
def test_every_capability_route_accepts_payload():
    ...
    if resp.status_code != 200:
        ...
        failures.append('record_check_in rejected a payload built from its own schema: ' + str(resp.json().get('error')))
    else:
        listed = client.get("/v1/record_check_in", headers=AUTH)
        ...
        if listed.status_code != 200:
            ...
            failures.append('record_check_in list refused: ' + str(listed_body.get('error') or listed_body)[:200])
        else:
            rows = _listed(listed_body)
            if not rows:
                failures.append('record_check_in accepted a record but persisted nothing')
            else:
                item_id = rows[0].get("id")
                got = client.get(f"/v1/record_check_in/{item_id}", headers=AUTH)
                if got.status_code != 200:
                    failures.append('record_check_in get: HTTP ' + str(got.status_code))
                missing = client.get("/v1/record_check_in/999999", headers=AUTH)
                if missing.status_code != 404:
                    failures.append('record_check_in missing id: HTTP ' + str(missing.status_code) + ' (expected 404)')
