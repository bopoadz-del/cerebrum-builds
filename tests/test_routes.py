...
def test_every_capability_route_answers():
    ...
    failures = []
    payload = {}
    resp = client.post("/v1/productivity_core", json=payload, headers=AUTH)
    if resp.status_code != 200:
        failures.append('productivity_core: HTTP ' + str(resp.status_code) + ': ' + resp.text[:200])
    else:
        try:
            body = resp.json()
        except Exception:
            failures.append('productivity_core: response is not JSON')
        else:
            if not isinstance(body, dict):
                failures.append('productivity_core: JSON body is not a dict')
            listed = client.get("/v1/productivity_core")
            if listed.status_code != 200:
                failures.append('productivity_core list: HTTP ' + str(listed.status_code))
...
def test_every_capability_route_accepts_payload():
    ...
    if resp.status_code != 200:
        ...
        failures.append('productivity_core rejected a payload built from its own schema: ' + str(resp.json().get('error')))
    else:
        listed = client.get("/v1/productivity_core")
        ...
        if listed.status_code != 200:
            ...
            failures.append('productivity_core list refused: ' + str(listed_body.get('error') or listed_body)[:200])
        else:
            rows = _listed(listed_body)
            if not rows:
                failures.append('productivity_core accepted a record but persisted nothing')
            else:
                item_id = rows[0].get("id")
                got = client.get(f"/v1/productivity_core/{item_id}")
                if got.status_code != 200:
                    failures.append('productivity_core get: HTTP ' + str(got.status_code))
                missing = client.get("/v1/productivity_core/999999")
                if missing.status_code != 404:
                    failures.append('productivity_core missing id: HTTP ' + str(missing.status_code) + ' (expected 404)')
