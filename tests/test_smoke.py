...
def test_capabilities_import():
    from app.actions import record_check_in
    assert record_check_in.CAPABILITY_ID
...
def test_dispatch_runs_offline():
    ...
    for block_id in ['dashboard', 'database', 'validation']:
        load_block(block_id)
...
def test_every_capability_handle_returns_mapping():
    ...
    from app.actions import record_check_in
    try:
        out = record_check_in.handle({'guest_name': 'sample', 'reference': 'sample', 'room_number': 'sample', 'status': 'open', 'checked_in_at': '2026-09-03T10:00:00', 'guests_count': 1, 'notes': 'sample'})
    except Exception as exc:
        out = {'ok': False, 'error': type(exc).__name__ + ': ' + str(exc)}
    if not isinstance(out, dict):
        failures.append('record_check_in handle() must return a dict, got ' + type(out).__name__)
    from app.actions import list_todays_check_ins
    try:
...
def test_every_capability_executes_end_to_end():
    ...
    out = record_check_in.handle({'guest_name': 'sample', 'reference': 'sample', 'room_number': 'sample', 'status': 'open', 'checked_in_at': '2026-09-03T10:00:00', 'guests_count': 1, 'notes': 'sample'})
    if not isinstance(out, dict):
        failures.append('record_check_in returned a non-dict: ' + repr(out)[:120])
    elif out.get("ok") is False:
        failures.append('record_check_in rejected a payload built from its own schema: ' + str(out.get('error')))
    elif '\"status\": \"error\"' in _json.dumps(out, default=str) or '\"status\": \"failed\"' in _json.dumps(out, default=str):
        failures.append('record_check_in reported ok around a failed block call: ' + _json.dumps(out, default=str)[:300])
    from app.actions import list_todays_check_ins
    out = list_todays_check_ins.handle({'check_in_date': '2026-09-03', 'guest_name': 'sample', 'reference': 'sample', 'room_number': 'sample', 'status': 'open', 'check_in_count': 1, 'notes': 'sample'})
    if not isinstance(out, dict):
