...
def test_capabilities_import():
    from app.actions import productivity_core
    assert productivity_core.CAPABILITY_ID
...
def test_every_capability_handle_returns_mapping():
    ...
    from app.actions import productivity_core
    try:
        out = productivity_core.handle({})
    except Exception as exc:
        out = {'ok': False, 'error': type(exc).__name__ + ': ' + str(exc)}
    if not isinstance(out, dict):
        failures.append('productivity_core handle() must return a dict, got ' + type(out).__name__)
    from app.actions import audit
    try:
        out = audit.handle({})
    except Exception as exc:
...
def test_every_capability_executes_end_to_end():
    ...
    out = productivity_core.handle({})
    if not isinstance(out, dict):
        failures.append('productivity_core returned a non-dict: ' + repr(out)[:120])
    elif out.get("ok") is False:
        failures.append('productivity_core rejected a payload built from its own schema: ' + str(out.get('error')))
    elif '\"status\": \"error\"' in _json.dumps(out, default=str) or '\"status\": \"failed\"' in _json.dumps(out, default=str):
        failures.append('productivity_core reported ok around a failed block call: ' + _json.dumps(out, default=str)[:300])
    from app.actions import audit
    out = audit.handle({})
    if not isinstance(out, dict):
