...
def test_the_whisper_goes_to_the_broker_leg_only():
    ...
    assert "<Number" in twiml and "+971509998888" in twiml
    assert "Warm lead on the line" in twiml
    assert "<Conference" in twiml
    assert twiml.index("Warm lead") < twiml.index("<Conference")
